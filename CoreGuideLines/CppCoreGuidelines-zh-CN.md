# [CppCoreGuidelines-zh-CN](http://lynnboy.github.io/CppCoreGuidelines-zh-CN/)
...
所有 C++ 程序员。其中也包括[考虑采用 C 语言的程序员](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#S-cpl)。
...
建议采用 [RAII](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rr-raii)：
...
- [I.22: 避免全局对象之间进行复杂的初始化](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Ri-global-init)
...
更好的做法是使用 [RAII](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rr-raii) 来在代码中保证后条件（“锁必须进行释放”）的实施：
...
**替代方案**: 如“这个资源必须被释放”这样形式的后条件最好以 [RAII](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rr-raii) 的方式来表达。

##### 注释

理想情况下，`Ensures` 应当是接口的一部分，但我们无法轻易做到这点。 当前，我们将之放入定义式（函数体）之中。 一旦语言支持变为可用（例如，参见[契约提案](http://www.open-std.org/jtc1/sc22/wg21/docs/papers/2016/p0380r1.pdf)），我们就将会采用前条件，后条件和断言的标准版本。
...
**参见**: [泛型编程](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#SS-gp)和[概念](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#SS-concepts)。
...
这种风格不幸地会导致未初始化的变量。 从 C++17 开始，可以使用 “结构化绑定” 功能特性来从返回值直接对多个变量初始化。
...
**参见**: [I.5](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Ri-pre) 和 [I.7](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Ri-post) 有关报告前条件和后条件的违反。
...
**替代方案**: 用“智能指针”来[传递所有权](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rr-smartptrparam)，比如 `unique_ptr`（专有所有权）和 `shared_ptr`（共享所有权）。 这样做比返回对象自身来说并没有那么简炼，而且通常也不那么高效， 因此，仅当需要引用语义时再使用智能指针。

**替代方案**: 有时候因为 ABI 兼容性的要求或者缺少资源，是无法对老代码进行修改的。 这种情况下，请用[指导方针支持库](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#gsl-guidelines-support-library)的 `owner` 来标记拥有对象的指针：
...
**参见**: [实参传递](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rf-conventional)，[使用智能指针参数](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rr-smartptrparam)，以及[返回值](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rf-value-return)。
...
`not_null` 在[指导方针支持库](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#gsl-guidelines-support-library)中定义。
...
当由 `q` 所指向的数组少于 `n` 个元素会怎么样？此时我们将覆写一些可能无关的内存。 当由 `p` 所指向的数组少于 `n` 个元素会怎么样？此时我们将读取一些可能无关的内存。 此二者都是未定义的行为，而且可能是非常恶劣的 BUG。
...
使用 `zstring` 和 `czstring` 来表示 C 风格的以零终结字符串。 但这样做时，应当使用 `std::string_view` 或 [GSL](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#gsl-guidelines-support-library) 中的 `span<char>` 以避免范围错误。
...
由于 `x` 和 `y` 是处于不同翻译单元之内的，调用 `f()` 和 `g()` 的顺序就是未定义的； 我们可能会访问到还未初始化的 `const` 对象。 这里展示的是，全局（命名空间作用域）对象的初始化顺序难题并不仅限于全局*变量*而已。
...
其中有四个模板参数和六个函数参数。 为简化最常用和最简单的用法，比较器参数可以缺省使用 `<`：
...
这违反了[避免未初始化变量](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Res-always)， [避免忽略所有权](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Ri-raw)， 和[避免魔法常量](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Res-magic)等规则。 尤其是，人们必须记得找地方写

```
if (owned) delete inp;
```

我们可以通过使用带有一个特殊的删除器（对 `cin` 不做任何事）的 `unique_ptr` 来处理这个特定的例子， 但这对于新手来说较复杂（他们很容易遇到这种问题），并且这个例子其实是一个更一般的问题的特例： 我们希望将其当做静态的某种属性（此处为所有权），需要在运行时进行 偶尔的处理。 一般的，更常见的，且更安全的例子可以被静态处理，因而我们并不希望为它们添加开销和复杂性。 然而我们还是不得不处理那些不常见的，较不安全的，而且更为昂贵的情况。 [[Str15\]](http://www.stroustrup.com/resource-model.pdf) 中对这种例子有所探讨。
...
- [F.1: 把有意义的操作“打包”成为精心命名的函数](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rf-package)
- [F.2: 一个函数应当实施单一一项逻辑操作](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rf-logical)
- [F.3: 保持函数短小简洁](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rf-single)
- [F.4: 如果函数可能必须在编译期进行求值，就将其声明为 `constexpr`](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rf-constexpr)
- [F.5: 如果函数非常小，并且是时间敏感的，就将其声明为 `inline`](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rf-inline)
- [F.6: 如果函数必然不会抛出异常，就将其声明为 `noexcept`](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rf-noexcept)
- [F.7: 对于常规用法，应当接受 `T*` 或 `T&` 参数而不是智能指针](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rf-smart)
- [F.8: 优先采用纯函数](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rf-pure)
- [F.9: 未使用的形参应当没有名字](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rf-unused)
- [F.10: 若操作可被重用，则应为其命名](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rf-name)
- [F.11: 当需要仅在一处使用的简单函数对象时使用无名 lambda](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/[#Rf-lambda)

参数传递表达式的规则：

- [F.15: 优先采用简单的和传统的信息传递方式](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rf-conventional)
- [F.16: 对于“输入（in）”参数，把复制操作廉价的类型按值进行传递，把其他类型按 `const` 引用进行传递](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rf-in)
- [F.17: 对于“输入/输出（in-out）”参数，按非 `const` 引用进行传递](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rf-inout)
- [F.18: 对于“将被移动（will-move-from）”参数，按 `X&&` 进行传递并对参数 `std::move`](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rf-consume)
- [F.19: 对于“转发（forward）”参数，按 `TP&&` 进行传递并只对参数 `std::forward`](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rf-forward)
- [F.20: 对于“输出（out）”值，采用返回值优先于输出参数](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rf-out)
- [F.21: 要返回多个“输出”值，优先返回结构体](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rf-out-multi)
- [F.60: 当“没有参数”是有效的选项时，采用 `T*` 优先于 `T&`](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rf-ptr-ref)

参数传递语义的规则：

- [F.22: 用 `T*` 或 `owner` 来代表单个对象](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rf-ptr)
- [F.23: 用 `not_null` 来表明“空值（null）”不是有效的值](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rf-nullptr)
- [F.24: 用 `span` 或者 `span_p` 来代表一个半开序列](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rf-range)
- [F.25: 用 `zstring` 或者 `not_null` 来代表 C 风格的字符串](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rf-zstring)
- [F.26: 当需要指针时，用 `unique_ptr` 来传递所有权](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rf-unique_ptr)
- [F.27: 用 `shared_ptr` 来共享所有权](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rf-shared_ptr)

值返回语义的规则：

- [F.42: 返回 `T*` 来（仅仅）给出一个位置](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rf-return-ptr)
- [F.43: 不要（直接或间接）返回指向局部对象的指针或引用](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rf-dangle)
- [F.44: 当不想进行复制，且不需要“没有对象被返回”时，返回 `T&`](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rf-return-ref)
- [F.45: 不要返回 `T&&`](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rf-return-ref-ref)
- [F.46: `int` 是 `main()` 的返回类型](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rf-main)
- [F.47: 赋值运算符返回 `T&`](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rf-assignment-op)
- [F.48: 不要用 `std::move(local)`](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rf-return-move-local)
- [F.49: 不要返回 `const T`](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rf-return-const)

其他函数规则：

- [F.50: 当函数不适用时（不能俘获局部变量，或者不能编写局部函数），就使用 Lambda](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rf-capture-vs-overload)
- [F.51: 如果需要作出选择，采用默认实参应当优先于进行重载](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rf-default-args)
- [F.52: 对于局部使用的（也包括传递给算法的）lambda，优先采用按引用俘获](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rf-reference-capture)
- [F.53: 对于非局部使用的（包括被返回的，在堆上存储的，或者传递给别的线程的）lambda，避免采用按引用俘获](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rf-value-capture)
- [F.54: 当俘获了 `this` 时，显式俘获所有的变量（不使用默认俘获）](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rf-this-capture)
- [F.55: 不要使用 `va_arg` 参数](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#F-varargs)
- [F.56: 避免不必要的条件嵌套](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#F-nesting)

函数和 Lambda 表达式以及函数对象有很强的相似性。

**参见**：[C.lambdas: 函数对象和 lambda](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#SS-lambdas)
...
循环体，包括用作循环体的 lambda，很少需要进行命名。 然而，大型的循环体（比如好多行或者好多页）也是个问题。 规则“[保持函数短小简洁](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rf-single)”暗含有“保持循环体短小”。 与此相似，用作回调参数的 lambda 有时候也是有意义的，虽然它们不大可能被重用。

##### 强制实施

- 参见“[保持函数短小简洁](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rf-single)”
...
如果 `collect()` 耗光了内存，程序就会崩溃。 除非这个程序特别精心编写成不会耗尽内存，否则这也许正是正确的方式； `terminate()` 能够产生合适的错误日志信息（但当内存耗尽时是很难做出任何巧妙的事情的）。

##### 注解

当你想决定是否要给函数标上 `noexcept` 时，一定要特别 注意你的代码的执行环境，尤其是与抛出异常和内存分配 相关的情形。打算成为完全通用的代码（比如像 标准库和其他类似的工具代码），应当支持那些 可以有意义地处理 `bad_alloc` 异常的执行环境。 不过，大多数程序和执行环境都不能有意义地 处理内存分配失败，而中止程序则是在这些情况中 应对分类失败的最干净和最简单的方式。如果已知 应用程序代码无法应对分配失败的话，对于即使 确实会进行分配的函数，添加 `noexcept` 也是适当的。
...
析构函数，`swap` 函数，移动操作，以及默认构造函数不应当抛出异常。 另请参见 [C.44](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rc-default00)。
...
我们可以静态地找出悬挂指针的许多常见情况（参见[生存期安全性剖面配置](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#SS-lifetime)）。函数实参天然存活于函数调用的生存期，因而具有更少的生存期问题。

##### 强制实施

- 【简单】若函数接受可复制的智能指针类型（即重载了 `operator->` 或 `operator*`），但该函数仅调用了：`operator*`、`operator->` 或 `get()`，则给出警告。 建议代之以 `T*` 或 `T&`。
...
- [当“无实参”是有效情形时，优先采用 `T*` 而不是 `T&`](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rf-ptr-ref)
- [智能指针规则概述](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rr-summary-smartptrs)
...
如果形参是根据条件不被使用的，可以用 `[[maybe_unused]]` 特性来声明它们。 例如：
...
- 作为[初始化](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#???)的 lambda。
...
![一般性参数传递表](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/images/param-passing-normal.png)

高级参数传递：

![高级参数传递表](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/images/param-passing-advanced.png)

只有在进行论证必要之后再使用高级技巧，并将其必要性注明在代码注释中。

对于字符序列的传递，参见 [字符串](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#SS-string)。

##### 例外

使用 `shared_ptr` 等类型来表达共享所有权时，不应遵循指导方针 F.16-21， 而应遵循 [R.34](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rr-sharedptrparam-owner)，[R.35](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rr-sharedptrparam)，以及 [R.36](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rr-sharedptrparam-const)。
...
- 如果函数需要无条件地从参数进行移动，那就按 `&&` 来接受参数。参见 [F.18](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rf-consume)。
- 如果函数只是需要为其自己局部使用而保留实参的局部可修改副本，那么按值接受是没问题的。
- 如果函数需要保留参数的一个副本用以传递给另一个目标（其他函数，或存储到非局部位置中），那就在按 `const&` 传递参数（对于左值）之外， 添加一个按 `&&` 传递参数（对于右值）的重载，并在函数体中将之 `std::move` 到其目标之中。基本上，这个重载是“将被移动（will-move-from）”；参见 [F.18](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rf-consume)。
- 在特殊情况中，比如有多个“输入+复制”的参数时，考虑采用完美转发。参见 [F.19](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rf-forward)。
...
避免“为了效率”而按 `T&&` 来传递参数这类的“玄奥技巧”。 关于按 `&&` 传递带来性能好处的大多数传言都是假的或者是脆弱的（不过也请参考 [F.18](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rf-consume) 和 [F.19](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rf-forward)）。

##### 注解

可以假定引用都指代了某个有效对象（语言规则）。 “空引用”（正规地说）是不存在的。 如果要表示一个非强制的值，请使用指针，`std::optional`，或者一个用以代表“没有值”的特殊值。
...
使用 `shared_ptr` 等类型来表达共享所有权时，应遵循 [R.34](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rr-sharedptrparam-owner) 或 [R.36](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rr-sharedptrparam-const)， 取决于函数是否无条件地获取实参的引用。
...
一些用户定义和标准程序库的类型，如 `span<T>` 或迭代器等， 是[可廉价复制](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rf-in)的，并可按值传递， 这样做时具有可改动（in-out）引用语义：
...
注意，`std::move(v)` 使得 `store_somewhere()` 可以把 `v` 遗留为被移走的状态。 [这可能很危险](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rc-move-semantic)。
...
当“将被移动”的形参是 `shared_ptr` 时，应遵循 [R.34](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rr-sharedptrparam-owner)，并按值传递 `shared_ptr`。
...
当有多个值要返回时，[使用元组](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rf-out-multi)或者类似的多成员类型。
...
- 对于内部循环中的多次函数调用之间重用自带容量的对象（比如 `std::string` 和 `std::vector`）：[将其按照输入/输出参数处理，并按引用传递](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rf-out-multi)。
...
具有有意义名字的 `struct` 在现代 C++ 中更常见。 例子参见 `ranges::min_max_result`、`from_chars_result` 等其他类型。

##### 例外

有时候需要把对象传递给函数让其操纵它的状态。 这种情况下，按引用传递对象 [`T&`](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rf-inout) 通常是恰当的技巧。 显式传递一个输入/输出参数再让其作为返回值返回出来通常是没必要的。 例如：
...
这里，`s` 和 `in` 都用作了输入/输出参数。 `in` 按（非 `const`）引用来传递，以便可以操作其状态。 `s` 的传递是为避免重复进行分配。 通过重用 `s`（按引用传递），我们只需要在为扩展 `s` 的容量时才会分配新的内存。 这种技巧有时候被称为“调用方分配的输出参数”模式，它特别适合于 诸如 `string` 和 `vector` 这样需要进行自由存储分配的类型。
...
大多数情况下，返回某种用户定义的某个专门的类型是有好处的。 例如：
...
请注意这与 [ES.56](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Res-move) 的 `return move(...)` 反模式是不同的。
...
构造出一个本质上是 `nullptr` 的引用是可能的，但不是合法的 C++ 代码（比如，`T* p = nullptr; T& r = *p;`）。 这种错误非常罕见。
...
**参见**: [支持程序库](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#gsl-guidelines-support-library)

**参见**: [请勿将数组作为单个指针来传递](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Ri-array)
...
清晰性。以 `not_null<T>` 为参数的函数很明确地说明，应当由该函数的调用者来负责进行任何必要的 `nullptr` 检查。 相似地，以 `not_null<T>` 为返回值的函数很明确地说明，该函数的调用者无须检查 `nullptr`。

##### 示例

`not_null<T*>` 让读者（人类或机器）明了，在进行解引用前不需要检查 `nullptr`。 而且当进行调试时，可以对 `owner<T*>` 和 `not_null<T>` 进行植入来进行正确性检查。
...
范围（Range）在 C++ 代码中十分常见。典型情况下，它们都是隐含的，且非常难于保证它们能够被正确使用。 特别地，给定一对儿参数 `(p, n)` 来代表数组 `[p:p+n)`， 通常来说不可能确定 `*p` 后面是不是真的存在 `n` 个元素。 `span<T>` 和 `span_p<T>` 两个简单的辅助类，分别用于代表范围 `[p:q)`，以及一个以 `p` 开头并以使谓词为真的第一个元素结尾的范围。
...
**参见**：[C.50](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rc-factory)关于何时从一个工厂中返回 `shared_ptr`。
...
如果同时不可能超过一个所有者的话，优先采用 `unique_ptr` 而不是 `shared_ptr`。 `shared_ptr` 的作用是共享所有权。
...
迭代器、索引值和引用也可以用来传递位置。 [当不需要使用 `nullptr`](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rf-ptr-ref)，或者[当不会改变被指代的对象](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#S-const)时，用引用通常比用指针更好。

##### 注解

不要返回指向某个不在调用方的作用域中的东西的指针；参见 [F.43](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rf-dangle)。

**参见**: [有关如何避免悬挂指针的讨论](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#???)
...
**参见**: 另一种获得悬挂指针的方式是[指针失效](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#???)。 这种情况也可以用相似的技术来检测和避免。
...
**参见**: 所返回的引用诀不能蕴含所有权的传递： [有关如何避免悬挂指针的讨论](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#???)以及[有关所有权的讨论](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#???)。
...
[生存期安全性](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#SS-lifetime)完全执行时，会捕捉到这个问题。

##### 示例

当临时的引用”向下”传递给被调用对象时，返回右值引用是正常的； 然后，临时对象保证比函数调用生命期更长（参见 [F.18](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rf-consume) 和 [F.19](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rf-forward)）。 但是，将这样的引用“向上”传递给更大的调用范围时，不好。 对于通过普通引用或完美传递方式传递参数，并希望返回值的通过函数，使用简单的 `auto` 而不是 `auto &&` 返回推导的类型。
...
我们提出这条规则，只是因为这种错误持续存在于大众之间。 注意，虽然其返回类型不是 `void`，但主函数并不需要明确的返回语句。
...
历史上曾有过一些建议让赋值运算符返回 `const T&`。 这主要是为了避免 `(a = b) = c` 形式的代码 —— 这种代码其实并不常见到足以成为违反与标准类型之间一致性的理由。
...
**另见**: [F.20，有关“out”输出值的一般条款](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rf-out)
...
- [C.1: 把相关的数据组织到结构中（`struct` 或 `class`）](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rc-org)
- [C.2: 当类具有不变式时使用 `class`；当数据成员可以独立进行变动时使用 `struct`](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rc-struct)
- [C.3: 用类来表示接口和实现之间的区别](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rc-interface)
- [C.4: 仅当函数直接访问类的内部表示时才让函数作为其成员](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rc-member)
- [C.5: 把辅助函数放在其所支持的类相同的命名空间之中](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rc-helper)
- [C.7: 不要在同一个语句中同时定义类或枚举并声明该类型的变量](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rc-standalone)
- [C.8: 当有任何非公开成员时使用 `class` 而不是 `struct`](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rc-class)
- [C.9: 让成员的暴露最小化](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rc-private)

子章节：

- [C.concrete: 具体类型](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#SS-concrete)
- [C.ctor: 构造函数，赋值和析构函数](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#S-ctor)
- [C.con: 容器和其他资源包装](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#SS-containers)
- [C.lambdas: 函数对象和 lambda](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#SS-lambdas)
- [C.hier: 类层次（OOP）](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#SS-hier)
- [C.over: 重载和运算符重载](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#SS-overload)
- [C.union: 联合体](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#SS-union)
...
- [把带有私有数据的类定义为 `class`](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rc-class)
- [优先将接口部分放在类的开头](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rl-order)
- [使成员的暴露最小化](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rc-private)
- [避免 `protected` 数据](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rh-protected)
...
使用这样的类来表示接口和实现之间的区别当然不是唯一可能的方式。 比如说，我们也可以使用命名空间中的一组自由函数，一个抽象基类，或者一个带有概念的函数模板来表示一个接口。 最重要的一点，在于明确地把接口和其实现“细节”区分开来。 理想地，并且典型地，接口要比其实现稳定得多。
...
这点对于[重载运算符](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Ro-namespace)来说尤其重要。
...
优先将接口部分放在类的开头，[参见 NL.16](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rl-order)。
...
无论我们在 `//` 部分中干什么，`pair` 的任意用户都可以任意地并且不相关地改动其 `a` 和 `b`。 在大型代码库中，我们无法轻易找出哪段代码对 `pair` 的成员都做了什么。 这也许正是我们想要的，但如果想要在成员之间强加某种关系，就需要使它们为 `private`， 并通过构造函数和成员函数来强加这种关系（不变式）。 例如：
...
如果无法轻易确定一组变量的直接用户的集合，那么这个集合的类型或用法也无法被（轻易）改变或改进。 对于 `public` 和 `protected` 数据来说这是常见的情况。
...
[`protected` 数据不是好主意](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rh-protected)。

##### 注解

优先让 `public` 成员在前，`protected` 成员其次，`private` 成员在后；参见 [NL.16](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rl-order)。

##### 强制实施

- [标记 `protected` 数据](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rh-protected)。
...
- [C.10: 优先使用具体类型而不是类继承层次](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rc-concrete)
- [C.11: 使具体类型正规化](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rc-regular)
- [C.12: 不要令可复制或移动类型的数据成员为 `const` 或引用](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rc-constref)
...
- [C.20: 只要可能，请避免定义任何的默认操作](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rc-zero)
- [C.21: 如果定义或者 `=delete` 了任何复制、移动或析构函数，请定义或者 `=delete` 它们全部](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rc-five)
- [C.22: 使默认操作之间保持一致](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rc-matched)

析构函数的规则：

- [C.30: 如果一个类需要在对象销毁时执行明确的操作，请为其定义析构函数](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rc-dtor)
- [C.31: 类所获取的所有资源，必须都在类的析构函数中进行释放](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rc-dtor-release)
- [C.32: 如果类中带有原始指针（`T*`）或者引用（`T&`），请考虑它是否是所有者](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rc-dtor-ptr)
- [C.33: 如果类中带有所有权的指针成员，请定义析构函数](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rc-dtor-ptr2)
- [C.35: 基类的析构函数应当要么是 public 和 virtual，要么是 protected 且非 virtual](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rc-dtor-virtual)
- [C.36: 析构函数不能失败](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rc-dtor-fail)
- [C.37: 使析构函数 `noexcept`](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rc-dtor-noexcept)

构造函数的规则：

- [C.40: 如果类具有不变式，请为其定义构造函数](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rc-ctor)
- [C.41: 构造函数应当创建经过完整初始化的对象](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rc-complete)
- [C.42: 当构造函数无法构造有效对象时，应当抛出异常](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rc-throw)
- [C.43: 保证可复制类带有默认构造函数](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rc-default0)
- [C.44: 尽量让默认构造函数简单且不抛出异常](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rc-default00)
- [C.45: 不要定义仅对数据成员进行初始化的默认构造函数；应当使用成员初始化式](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rc-default)
- [C.46: 默认情况下，把单参数的构造函数声明为 `explicit`](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rc-explicit)
- [C.47: 按成员声明的顺序对数据成员进行定义和初始化](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rc-order)
- [C.48: 对于常量初始化式来说，优先采用默认成员初始化式而不是构造函数中的成员初始化式](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rc-in-class-initializer)
- [C.49: 优先进行初始化而不是在构造函数中赋值](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rc-initialize)
- [C.50: 当初始化过程中需要体现“虚函数行为”时，请使用工厂函数](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rc-factory)
- [C.51: 用委派构造函数来表示类中所有构造函数的共同行为](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rc-delegating)
- [C.52: 使用继承构造函数来把构造函数引入到无须进行其他的明确初始化操作的派生类之中](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rc-inheriting)

复制和移动的规则：

- [C.60: 使复制赋值非 `virtual`，接受 `const&` 的参数，并返回非 `const` 的引用](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rc-copy-assignment)
- [C.61: 复制操作应当进行复制](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rc-copy-semantic)
- [C.62: 使复制赋值可以安全进行自赋值](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rc-copy-self)
- [C.63: 使移动赋值非 `virtual`，接受 `&&` 的参数，并返回非 `const&`](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rc-move-assignment)
- [C.64: 移动操作应当进行移动，并使原对象处于有效状态](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rc-move-semantic)
- [C.65: 使移动赋值可以安全进行自赋值](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rc-move-self)
- [C.66: 使移动操作 `noexcept`](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rc-move-noexcept)
- [C.67: 多态类应当抑制公开的移动/复制操作](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rc-copy-virtual)

其他的默认操作规则：

- [C.80: 当需要明确使用缺省语义时，使用 `=default`](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rc-eqdefault)
- [C.81: 当需要关闭缺省行为（且不需要替代的行为）时，使用 `=delete`](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rc-delete)
- [C.82: 不要在构造函数和析构函数中调用虚函数](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rc-ctor-virtual)
- [C.83: 考虑为值类型提供 `noexcept` 的 `swap` 函数](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rc-swap)
- [C.84: `swap` 不能失败](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rc-swap-fail)
- [C.85: 使 `swap` 函数 `noexcept`](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rc-swap-noexcept)
- [C.86: 使 `==` 对操作数的类型对称，并使之 `noexcept`](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rc-eq)
- [C.87: 请当心基类的 `==`](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rc-eq-base)
- [C.89: 使 `hash` 函数 `noexcept`](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rc-hash)
- [C.90: 依靠构造函数和赋值运算符，不要依靠 `memset` 和 `memcpy`](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rc-memset)
...
为避免发生如 [C.67](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rc-copy-virtual) 所说的切片， 使复制和移动操作为受保护的或 `=delete`，并添加 `clone`：
...
一个小错误（例如拼写错误，遗漏 `const`，使用 `&` 而不是 ‘&&`，或遗漏一个特殊功能）可能导致错误或警告。 为避免单调乏味和出错的可能性，请尝试遵循[零规则](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/＃Rc-zero)。
...
“这个类需要析构函数吗？”是一个出人意料有洞察力的设计问题。 对于大多数类来说，答案是“不需要”，要么是因为类中并没有保持任何资源，要么是因为销毁过程已经被[零之准则](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rc-zero)处理掉了； 就是说，它的成员在销毁之中可以自己照顾自己。 当答案为“需要”时，类的大部分设计应当遵循下列规则（参见[五之准则](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rc-five)）。
...
不过关不掉的 socket 怎么办呢？析构函数、close 以及清理操作[不应当失败](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rc-dtor-fail)。 如果它确实这样的话，就出现了一个不存在真正的好解决方案的问题。 对于新手来说，作为析构函数的编写者，无法了解析构函数是因为什么被调用的，而且不能通过抛出异常来“拒绝执行”。 参见[相关讨论](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Sd-never-fail)。 让这个问题更加糟糕的，还包括许多的 close/release 操作都是无法重试的。 许多人都曾试图解决这个问题，但仍不存在已知的一般性解决方案。 如果可能的话，可以考虑把 close/cleanup 的失败看成是基本的设计错误，然后终止程序（terminate）。
...
- 【简单】 当类中所有的指针或引用成员是所有者 （比如通过使用 `gsl::owner` 所断定）时，它们就应当在析构函数中有所引用。
- 【困难】 当在所有权上没有明确的说法时，为指针或引用成员确定其是否是所有者 （比如，检查构造函数的代码）。
...
所有权在新代码（以及重构的遗留代码）中应当是清晰的：[R.20](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rr-owner) 对于有所有权指针， [R.3](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rr-ptr) 对于无所有权指针。引用从来不能有所有权，[R.4](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rr-ref)。
...
指针成员可以表示某种资源。 [不应该这样使用 `T*`](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rr-ptr)，但老代码中这是很常见的。 请把 `T*` 当作一种可能的所有者的嫌疑。
...
注意，当你定义析构函数时，你必须定义或者弃置（delete）[所有的默认操作](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rc-five)：
...
请参见[这条规则](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Sd-dtor)中的探讨段落.
...
许多人都曾试图针对析构函数中的故障处理设计一种傻瓜式的方案。 但没有人得到过任何一种通用方案。 这确实是真正的实际问题：比如说，怎么处理无法关闭的 socket？ 析构函数的编写者无法了解析构函数为什么会被调用，并且不能通过抛出异常来“拒绝执行”。 参见[探讨](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Sd-never-fail)段落。 让问题更麻烦的是，许多的“关闭/释放”操作还都是不能重试的。 如果确实可行的话，请把“关闭/清理”的失败作为一项基本设计错误并终止（terminate）程序。
...
[析构函数不能失败](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rc-dtor-fail)。如果析构函数试图抛出异常来退出，这就是一种设计错误，程序最好终止执行。
...
`Rec2` 的构造函数是多余的。 同样的，`int` 的默认值最好用[默认成员初始化式](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rc-in-class-initializer)来给出。

**参见**: [构造有效对象](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rc-complete)和[构造函数抛出异常](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rc-throw)。
...
如果无法方便地通过构造函数来构造有效的对象的话，请[使用工厂函数](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rc-factory)。
...
如果构造函数（为创建有效的对象）获取了某个资源，则这个资源应当[由析构函数释放](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rc-dtor-release)。 这种以构造函数获取资源并以析构函数来释放的惯用法被称为 [RAII](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rr-raii)（“资源获取即初始化/Resource Acquisition Is Initialization”）。
...
有些领域，比如像飞行器控制这样的硬实时系统中，（在没有其他工具支持下）异常处理在计时方面不具有足够的可预测性。 这样的话就必须使用 `is_valid()` 技巧。这种情况下，可以一贯并即刻地检查 `is_valid()` 来模拟 [RAII](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rr-raii)。

##### 替代方案

如果你觉得想要使用某种“构造函数之后初始化”或者“两阶段初始化”手法，请试着避免这样做。 如果你确实要如此的话，请参考[工厂函数](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rc-factory)。

##### 注解

人们使用 `init()` 函数而不是在构造函数中进行初始化的一种原因是为了避免代码重复。 [委派构造函数](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rc-delegating)和[默认成员初始化式](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rc-in-class-initializer)可以更好地做到这点。 另一种原因是为了把初始化推迟到需要对象的位置；它的解决方法通常为“[直到变量可以正确进行初始化的位置再声明变量](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Res-init)”。
...
许多的语言和程序库设施都依赖于默认构造函数来初始化其各个元素，比如 `T a[10]` 和 `std::vector<T> v(10)`。 对于同时是可复制的类型来说，默认构造函数通常会简化定义一个适当的[移动遗留状态](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#???)的任务。
...
**参见**: [有关隐式转换的讨论](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Ro-conversion)
...
**参见**: [讨论](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Sd-order)
...
可以使用 C++17 的 `std::string_view` 或 `gsl::span<char>` 代替这些 `const char*` 来作为[一种表示函数实参的更通用的方式](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rstr-view)：
...
`make_shared` 要求公开的构造函数。构造函数通过要求一个受保护的 `Token` 而无法再被公开调用，从而避免不完全构造的对象泄漏出去。 通过提供工厂函数 `create()`，（在自由存储上）构造对象变得简便。

##### 注解

根据惯例，工厂方法在自由存储上进行分配，而不是在运行栈或者某个外围对象之内进行。

**参见**: [讨论](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Sd-factory)
...
**参见**: 当“重复行为”是简单的初始化时，考虑使用[默认成员初始化式](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rc-in-class-initializer)。
...
具体类型一般都应当是可以复制的，而类层次中的接口则不应如此。 资源包装可以复制也可以不能复制。 我们可以基于逻辑因素，也可以为性能原因而将类型定义为可移动的。

### C.60: 使复制赋值非 `virtual`，接受 `const&` 的参数，并返回非 `const` 的引用

##### 理由

这样做简单且高效。如果想对右值进行优化，则可以提供一个接受 `&&` 的重载（参见 [F.18](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rf-consume)）。
...
`swap` 实现技巧可以提供[强保证](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Abrahams01)。
...
直接向目标元素中进行写入的话，我们得到的是[基本保证](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Abrahams01)而不是 `swap` 技巧所提供的强保证。还要当心[自赋值](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rc-copy-self)。

**替代方案**: 如果你想要 `virtual` 的赋值运算符，并了解为何这样做很有问题的话，请不要使其为 `operator=`。请使用一个命名函数，如 `virtual void assign(const Foo&)`。 参见[复制构造函数 vs. `clone()`](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rc-copy-virtual)。
...
可以通过明确检测自赋值来处理自赋值的情况，不过通常不进行这种检测会变得更快并且更优雅（比如说，[利用 `swap`](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rc-swap)）。
...
**参见**: [针对复制赋值的规则](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rc-copy-assignment)。

##### 强制实施

和针对[复制赋值](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rc-copy-assignment)所做的相同。
...
[自赋值](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rc-copy-self)中反对 `if (this == &a) return *this;` 测试的“每一百万次有一次”的论点，在自移动的情况中更加适当。
...
当需要创建多态对象的深拷贝副本时，应当使用 `clone()` 函数：参见 [C.130](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rh-copy)。
...
除了语言为之提供默认实现的操作之外， 还有一些操作也是非常基础的，需要对它们的定义给出专门的规则： 比较，`swap`，以及 `hash`。
...
**参见** [工厂函数](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rc-factory)，以了解如何获得调用派生类函数的效果又不会引发未定义行为。
...
`swap` 对于实现许多惯用法都很有用，其范围包括从平滑地进行对象移动，到轻易实现提供了受保证的提交功能的赋值操作以允许编写具有强异常安全性的调用代码。考虑利用 `swap` 来基于复制构造实现复制赋值操作。另见[析构函数，回收，以及 `swap` 不允许失败](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Re-never-fail)。
...
[`swap` 不能失败](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rc-swap-fail)。 如果 `swap` 试图用异常来退出的话，这就是严重的设计错误，程序最好立即终止（terminate）。
...
构造某个类型的实例的标准 C++ 机制是调用其构造函数。如指导方针 [C.41](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rc-complete) 所述：构造函数应当创建一个已完全初始化的对象。不应当需要进行如用 `memcpy` 来进行的额外初始化。 为适当地做出一个类的副本并维持类型的不变式，类型将提供复制构造函数和/或复制赋值运算符。使用 `memcpy` 来复制一个非可平凡复制的类型具有未定义的行为。这经常会导致切片，或者数据损坏。
...
- [C.100: 定义容器的时候要遵循 STL](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rcon-stl)
- [C.101: 为容器提供值语义](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rcon-val)
- [C.102: 为容器提供移动操作](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rcon-move)
- [C.103: 为容器提供一个初始化式列表构造函数](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rcon-init)
- [C.104: 为容器提供一个将之置空的默认构造函数](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rcon-empty)
- ???
- [C.109: 当资源包装类具有指针语义时，应提供 `*` 和 `->`](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rcon-ptr)

**参见**: [资源](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#S-resource)
...
函数对象是提供了重载的 `()` 的对象，因此可以进行调用。 Lambda 表达式（通常通俗地简称为“lambda”）是一种产生函数对象的写法。 函数对象应当可廉价地复制（因此可以[按值传递](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rf-in)）。

概要：

- [F.10: 若操作可被重用，则应为其命名](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rf-name)
- [F.11: 当需要仅在一处使用的简单函数对象时使用无名 lambda](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/[#Rf-lambda)
- [F.50: 无法用函数达成（捕捉局部变量，或者编写局部函数）时，应使用 lambda](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rf-capture-vs-overload)
- [F.52: 在将被局部范围内使用（包括将之传递给算法）的 lambda 中优先按引用捕捉](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rf-reference-capture)
- [F.53: 在不被局部范围内使用（包括存储在堆上，或传递给其他线程）的 lambda 中避免按引用捕捉](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rf-value-capture)
- [ES.28: 针对复杂的初始化（尤其是 `const` 变量）使用 lambda](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Res-lambda-init)
...
- [C.120: 类层次（仅）用于表达具有天然层次化结构的概念](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rh-domain)
- [C.121: 如果基类被用作接口的话，应使其成为纯抽象类](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rh-abstract)
- [C.122: 当需要完全区分接口和实现时，应当用抽象类作为接口](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rh-separation)

类层次的设计规则概览：

- [C.126: 抽象类通常并不需要用户编写的构造函数](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rh-abstract-ctor)
- [C.127: 带有虚函数的类应当带有虚的或受保护的析构函数](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rh-dtor)
- [C.128: 虚函数应当指明 `virtual`、`override`、`final` 三者之一](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rh-override)
- [C.129: 当设计类层次时，应区分实现继承和接口继承](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rh-kind)
- [C.130: 多态类的深拷贝；优先采用虚函数 `clone` 来替代公开复制构造/赋值](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rh-copy)
- [C.131: 避免无价值的取值和设值函数](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rh-get)
- [C.132: 请勿无理由地使函数 `virtual`](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rh-virtual)
- [C.133: 避免 `protected` 数据](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rh-protected)
- [C.134: 确保所有非 `const` 数据成员有相同的访问级别](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rh-public)
- [C.135: 用多继承来表达多个不同的接口](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rh-mi-interface)
- [C.136: 用多继承来表达一些实现特性的合并](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rh-mi-implementation)
- [C.137: 用 `virtual` 基类以避免过于通用的基类](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rh-vbase)
- [C.138: 用 `using` 来为派生类和其基类建立重载集合](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rh-using)
- [C.139: 对类运用 `final` 应当保守](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rh-final)
- [C.140: 不要在虚函数和其覆盖函数上提供不同的默认参数](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rh-virtual-default-arg)

对类层次中的对象进行访问的规则概览：

- [C.145: 通过指针和引用来访问多态对象](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rh-poly)
- [C.146: 当无法避免在类层次上进行导航时应使用 `dynamic_cast`](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rh-dynamic_cast)
- [C.147: 当查找所需类的失败被当做一种错误时，应当对引用类型使用 `dynamic_cast`](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rh-ref-cast)
- [C.148: 当查找所需类的失败被当做一种有效的可能情况时，应当对指针类型使用 `dynamic_cast`](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rh-ptr-cast)
- [C.149: 用 `unique_ptr` 或 `shared_ptr` 来避免忘记对以 `new` 所创建的对象进行 `delete` 的情况](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rh-smart)
- [C.150: 用 `make_unique()` 来构建由 `unique_ptr` 所拥有的对象](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rh-make_unique)
- [C.151: 用 `make_shared()` 来构建由 `shared_ptr` 所拥有的对象](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rh-make_shared)
- [C.152: 禁止把指向派生类对象的数组的指针赋值给指向基类的指针](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rh-array)
- [C.153: 优先采用虚函数而不是强制转换](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rh-use-virtual)
...
带有虚函数的类通常是通过指向基类的指针来使用的。一般来说，最后一个使用者必须在基类指针上执行 `delete`，这常常是通过基类智能指针来做到的，因而析构函数应当为 `public` 和 `virtual`。而不那么常见的情况是当并不打算支持通过基类指针来删除时，这时析构函数应当为 `protected` 和非 `virtual`；参见 [C.35](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rc-dtor-virtual)。
...
纯粹的接口类只是一组纯虚函数；参见 [I.25](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Ri-abstract)。
...
- 向 `Shape` 添加新的数据成员（比如绘制风格或者画布） 将导致所有派生于 `Shape` 的类和所有使用 `Shape` 的代码都需要进行复审，可能需要修改，而且很可能需要重新编译。
...
分离接口和实现的另一个（相关的）技巧是 [Pimpl](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Ri-pimpl)。
...
由于切片的问题，不鼓励多态类的复制操作，参见 [C.67](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rc-copy-virtual)。如果确实需要复制语义的话，应当进行深复制：提供一个虚 `clone` 函数，它复制的是真正的最终派生类型，并返回指向新对象的具有所有权的指针，而且在派生类中它返回的也是派生类型（利用协变返回类型）。
...
通常来说，推荐使用智能指针来表示所有权（参见 [R.20](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rr-owner)）。不过根据语言规则，协变返回类型不能是智能指针：当 `B::clone` 返回 `unique_ptr<B>` 时 `D::clone` 不能返回 `unique_ptr<D>`。因此，你得在所有覆盖函数中统一都返回 `unique_ptr<B>`，或者也可以使用[指导方针支持库](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#SS-views)中的 `owner<>` 工具类。
...
注意，我们可以为数据成员提供默认初始化式：[C.49: 优先进行初始化而不是在构造函数中赋值](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rc-initialize).
...
受保护数据经常看起来倾向于允许通过派生来进行任意的改进。 而通常你得到的是肆无忌惮的改动和各种错误。 应当[优先采用 `private` 数据](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rc-private)并提供良好定义并强加的不变式。 或者通常更好的做法是，[不要在用作接口的任何类中存放数据](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rh-abstract)。
...
- *全 public*: 如果编写的是聚集一组变量而没有在这些变量之间维护不变式的话，所有这些变量都应当为 `public`。 [依照惯例，应当把这样的类声明为 `struct` 而不是 `class`](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rc-struct)
...
`istream` 提供了输入操作的接口（以及一些数据）；`ostream` 提供了输出操作的接口（以及一些数据）。 `iostream` 提供了 `istream` 和 `ostream` 接口的并集，以及在单个流上同时允许二者所需的同步操作。
...
有时候，“实现特性”更像是“混元”，决定实现的行为， 并向其中注入成员以使该实现提供其所要求的策略。 相关的例子可以参考 `std::enable_shared_from_this` 或者 boost.intrusive 中的各种基类（例如 `list_base_hook` 和 `intrusive_ref_counter`）。
...
很明显，这个例子过于“理论化”，但确实很难找到一个*小型*的现实例子出来。 `Interface` 是一个[接口层次](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rh-abstract)的根， 而 `Utility` 则是一个[实现层次](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rh-kind)的根。 [一个稍微更现实的例子](https://www.quora.com/What-are-the-uses-and-advantages-of-virtual-base-class-in-C%2B%2B/answer/Lance-Diduck)，有一些解释。
...
并非每个类都要作为基类的。 大多数标准库的类都是这样（比如，`std::vector` 和 `std::string` 都并非为了派生而设计）。 这条规则是关于在准备作为某个类层次的接口的带有虚函数的类中有关 `final` 的使用的。
...
[多态类应当抑制复制操作](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rc-copy-virtual)
...
和其他强制转换一样，`dynamic_cast` 被过度使用了。 [优先使用虚函数而不是强制转换](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rh-use-virtual)。 如果可行（无须运行时决议）并且相当便利的话，优先使用[静态多态](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#???)而不是 继承层次的导航。

##### 注解

一些人会在 `typeid` 更合适的时候使用 `dynamic_cast`； `dynamic_cast` 是一个通用的“是一个”操作，用以发现对象上的最佳接口， 而 `typeid` 是“报告对象的精确类型”操作，用以发现对象的真实类型。 后者本质上就是更简单的操作，因而应当更快一些。 后者（`typeid`）是可以在需要时进行手工模仿的（比如说，当工作在（因为某种原因）禁止使用 RTTI 的系统上）， 而前者（`dynamic_cast`）要正确地实现则要困难得多。
...
如果你所用的实现提供的 `dynamic_cast` 确实很慢的话，你可能只得使用一种替代方案了。 不过，所有无法静态决议的替代方案都涉及显式的强制转换（通常为 `static_cast`），而且易于出错。 你基本上都将会创建自己的特殊目的 `dynamic_cast`。 因此，首先应当确定你的 `dynamic_cast` 确实和你想的一样慢（其实有相当多的并不被支持的流言）， 而且你使用的 `dynamic_cast` 确实是性能十分关键的。

我们的观点是，当前的实现中的 `dynamic_cast` 并非很慢。 比如说，在合适的条件下，是可以以[快速常量时间](http://www.stroustrup.com/fast_dynamic_casting.pdf)来实施 `dynamic_cast` 的。 但是，兼容性问题使得作出改变很难，虽然大家都同意对优化付出的努力是值得的。
...
- 本条规则属于[类型安全性剖面配置](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Pro-type-downcast)。
...
与此相对，[C.147](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rh-ref-cast) 中失败即是错误，而且不能用于条件执行。

##### 示例

下面的例子的 `Shape_owner` 的 `add` 函数获取构造的 `Shape` 对象的所有权。这些对象也根据其几何特性被存储到了不同视图中。 这个例子中的 `Shape` 并不继承于 `Geometric_attributes`，而是其各个子类继承。
...
参见 [R.23](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rr-make_unique)。

### C.151: 用 `make_shared()` 来构建由 `shared_ptr` 所拥有的对象

参见 [R.22](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rr-make_shared)。
...
参见 [C.146](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rh-dynamic_cast) 和 ???
...
- [C.160: 定义运算符应当主要用于模仿传统用法](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Ro-conventional)
- [C.161: 对于对称的运算符应当采用非成员函数](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Ro-symmetric)
- [C.162: 重载的操作之间应当大体上是等价的](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Ro-equivalent)
- [C.163: 应当仅对大体上等价的操作进行重载](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Ro-equivalent-2)
- [C.164: 避免隐式转换运算符](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Ro-conversion)
- [C.165: 为定制点采用 `using`](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Ro-custom)
- [C.166: 一元 `&` 的重载只能作为某个智能指针或引用系统的一部分而提供](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Ro-address-of)
- [C.167: 应当为带有传统含义的操作提供运算符](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Ro-overload)
- [C.168: 应当在操作数所在的命名空间中定义重载运算符](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Ro-namespace)
- [C.170: 当想要重载 lambda 时，应当使用泛型 lambda](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Ro-lambda)
...
这里维持了传统的语义：[副本之间相等](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#SS-copy)。
...
非成员运算符应当要么是友元，要么定义于[其操作数所在的命名空间中](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Ro-namespace)。 [二元运算符应当等价地对待其两个操作数](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Ro-symmetric)。
...
但这也许并不是我们在泛型代码中所要的。 通常如果专门的函数存在的话我们就想用它，否则的话我们则需要通用的函数。 这是通过在函数的查找中包含通用函数而达成的：
...
如果你要“折腾”运算符 `&` 的话，请保证其定义和 `->`，`[]`，`*` 和 `.` 在结果类型上具有匹配的含义。 注意，运算符 `.` 现在是无法重载的，因此不可能做出一个完美的系统。 我们打算修正这一点： [Operator Dot (R2)](http://www.open-std.org/jtc1/sc22/wg21/docs/papers/2015/n4477.pdf)。 注意 `std::addressof()` 总会产生一个内建指针。
...
这里的 `!s` 的含义在 `N` 和 `M` 中是不同的。 这可能是最易混淆的一点。 移除 `namespace M` 的定义之后，混乱就被一种犯错的机会所代替。
...
这是这条规则的一种特殊情况：[辅助函数应当定义于它们的类相同的命名空间之中](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rc-helper)。
...
- [C.180: 采用 `union` 用以节省内存](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Ru-union)
- [C.181: 避免“裸” `union`](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Ru-naked)
- [C.182: 利用匿名 `union` 实现带标记联合体](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Ru-anonymous)
- [C.183: 不要将 `union` 用于类型双关](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Ru-pun)
- ???
...
但请你留意这个警告：[避免“裸”`union`](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Ru-naked)。
...
设计良好的带标记联合体是类型安全的。 *匿名*联合体可以简化这种带有 (tag, union) 对的类的定义。
...
不幸的是，`union` 经常被用于类型双关。 我们认为“它有时候能够按预期工作”并不是一种令人信服的理由。
...
- [Enum.1: 优先采用枚举而不是宏](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Renum-macro)
- [Enum.2: 采用枚举来表示相关的具名常量的集合](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Renum-set)
- [Enum.3: 优先采用 `enum class` 而不是“普通”`enum`](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Renum-class)
- [Enum.4: 针对安全和简单的用法来为枚举定义操作](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Renum-oper)
- [Enum.5: 请勿为枚举符采用 `ALL_CAPS` 命名方式](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Renum-caps)
- [Enum.6: 避免使用无名枚举](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Renum-unnamed)
- [Enum.7: 仅在必要时才为枚举指定其底层类型](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Renum-underlying)
- [Enum.8: 仅在必要时才指定枚举符的值](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Renum-value)
...
本章节中包含于资源相关的各项规则。 资源，就是任何必须进行获取，并（显式或隐式）进行释放的东西，比如内存、文件句柄、Socket 和锁等等。 其必须进行释放的原因在于它们是短缺的，因而即便是采用延迟释放也许也是有害的。 基本的目标是要确保不会泄漏任何资源，而且不会持有不在需要的任何资源。 负责释放某个资源的实体被称作是其所有者。
...
- 资源管理规则概览：
  - [R.1: 利用资源句柄和 RAII（资源获取即初始化）来自动管理资源](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rr-raii)
  - [R.2: 接口中的原生指针（仅）代表个体对象](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rr-use-ptr)
  - [R.3: 原生指针（`T*`）没有所有权](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rr-ptr)
  - [R.4: 原生引用（`T&`）没有所有权](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rr-ref)
  - [R.5: 优先采用有作用域的对象，避免不必要的堆分配](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rr-scoped)
  - [R.6: 避免非 `const` 的全局变量](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rr-global)
- 分配和回收规则概览：
  - [R.10: 避免 `malloc()` 和 `free()`](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rr-mallocfree)
  - [R.11: 避免显式调用 `new` 和 `delete`](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rr-newdelete)
  - [R.12: 显式资源分配的结果应当立即交给一个管理对象](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rr-immediate-alloc)
  - [R.13: 单个表达式语句中至多进行一次显式资源分配](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rr-single-alloc)
  - [R.14: 避免使用 `[\]` 形参，优先使用 `span`](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rr-ap)
  - [R.15: 总是同时重载相匹配的分配、回收函数对](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rr-pair)
- 智能指针规则概览：
  - [R.20: 用 `unique_ptr` 或 `shared_ptr` 表示所有权](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rr-owner)
  - [R.21: 优先采用 `unique_ptr` 而不是 `shared_ptr`，除非需要共享所有权](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rr-unique)
  - [R.22: 使用 `make_shared()` 创建 `shared_ptr`](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rr-make_shared)
  - [R.23: 使用 `make_unique()` 创建 `unique_ptr`](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rr-make_unique)
  - [R.24: 使用 `std::weak_ptr` 来打断 `shared_ptr` 的循环引用](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rr-weak_ptr)
  - [R.30: 以智能指针为参数，仅用于明确表达生存期语义](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rr-smartptrparam)
  - [R.31: 非 `std` 的智能指针，应当遵循 `std` 的行为模式](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rr-smart)
  - [R.32: `unique_ptr` 参数用以表达函数假定获得 `widget` 的所有权](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rr-uniqueptrparam)
  - [R.33: `unique_ptr&` 参数用以表达函数对该 `widget` 重新置位](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rr-reseat)
  - [R.34: `shared_ptr` 参数用以表达函数是所有者的一份子](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rr-sharedptrparam-owner)
  - [R.35: `shared_ptr&` 参数用以表达函数可能会对共享的指针重新置位](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rr-sharedptrparam)
  - [R.36: `const shared_ptr&` 参数用以表达它可能将保留一个对对象的引用 ???](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rr-sharedptrparam-const)
  - [R.37: 不要把来自某个智能指针别名的指针或引用传递出去](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rr-smartptrget)
...
这段代码中，你必须记得在所有路径中调用 `unlock`、`close_port` 和 `delete`，并且每个都恰好调用一次。 而且，一旦上面标有 `...` 的任何代码抛出了异常，`x` 就会泄漏，而 `my_mutex` 则保持锁定。
...
一旦发现一个“表现不良”的资源并未以带有析构函数的类来表示，就用一个类来包装它，或者使用 [`finally`](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Re-finally)。

**参见**: [RAII](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Re-raii)
...
当前许多的单元素指针的用法其实都应当用引用。 不过，如果 `nullptr` 是可能的值的话，也许引用就不是合理的替代方案了。
...
这些代码是无法被全部重写的（即便假定有良好的代码转换软件），尤其不会很快发生。 这个问题是不能简单通过把所有有所有权的指针都转换成 `unique_ptr` 和 `shared_ptr` 来（大规模）解决的， 这部分是因为我们确实需要在基础的资源句柄的实现中一起使用有所有权的“原生指针”和简单的指针。 例如，常见的 `vector` 实现中都有一个有所有权的指针和两个没有所有权的指针。 许多 ABI（以及基本上全部的面向 C 的接口代码）都使用 `T*`，其中不少都是有所有权的。 一些接口是无法简单地用 `owner` 来标记的，因为它们需要维持可以作为 C 来编译， （这可能是少见的恰当的使用宏的场合，它仅在 C++ 模式中扩展为 `owner`）。
...
除了遭受[资源泄漏](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rp-leak)的问题外，这也带来了一组假性的分配和回收操作，而这其实是不必要的。如果 Gadget 可以廉价地从函数转移出来（就是说，它很小，或者具有高效的移动操作）的话，直接“按值”返回即可（参见[输出返回值](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rf-out)）：
...
**参见**: [原生指针的规则](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rr-ptr)
...
- 【简单】 当局部的 `Unique_pointer` 或 `Shared_pointer` 在其生存期结束前未被移动、复制、赋值或 `reset`，则给出警告。 例外：不对指向无界数组的局部 `Unique_pointer` 产生这种警告。（见下文）
...
参见 [I.2](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Ri-global)
...
有些应用程序或者代码段是不能接受异常的。 它们的最佳例子就是那些性命攸关的硬实时代码。 但要注意的是，许多对异常的禁止其实是基于某种（不良的）迷信， 或者来源于对没有进行系统性的资源管理的老式代码库的关注（很不幸，但这经常是必须的）。 在这些情况下，应当考虑使用 `nothrow` 版本的 `new`。
...
大型程序中，裸露的 `delete`（即出现于应用程序代码中，而不是专门进行资源管理的代码中） 很可能是一个 BUG：如果已经有了 N 个 `delete` 的话，怎么确定我们需要的不是 N+1 或者 N-1 个呢？ 这种 BUG 可能会潜伏起来：它也许只会在维护过程中才暴露出来。 如果出现了裸露的 `new`，那就可能在别的什么地方需要一个裸露的 `delete`，因而可能也是一个 BUG。
...
【简单】 如果 `new` 的返回值被赋值给了原生指针，就给出警告。 【简单】 如果返回带所有权原始指针的函数的结果被赋值给了原生指针，就给出警告。
...
`make_unique` 为构造提供了更精炼的语句。 它也保证（C++17 前的代码中的）了复杂表达式中的异常安全性。
...
参见 [F.7](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rf-smart)。
...
上面两段根据 [`sharedptrparam` 指导方针](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rr-smartptrparam)来说都是错误的： `p` 是一个 `Shared_pointer`，但其共享性质完全没有被用到，而对其进行按值传递则是一种暗含的劣化； 这两个函数应当仅当它们需要参与 `widget` 的生存期管理时才接受智能指针。否则当可以为 `nullptr` 时它们就应当接受 `widget*`，否则，理想情况下，函数应当接受 `widget&`。 这些智能指针都符合 `Shared_pointer` 的概念，因此这些强制实施指导方针的规则可以直接应用，并使得这种一般性的劣化情况暴露出来。
...
有关命名、注释和缩进的规则，参见 [NL: 命名与代码布局](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#S-naming)。

一般规则：

- [ES.1: 优先采用标准库而不是其他的库或者“手工自制代码”](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Res-lib)
- [ES.2: 优先采用适当的抽象而不是直接使用语言功能特性](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Res-abstr)
- [ES.3: 避免重复（DRY），避免冗余代码](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Res-dry)

声明的规则：

- [ES.5: 保持作用域尽量小](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Res-scope)
- [ES.6: 在 for 语句的初始化式和条件中声明名字以限制其作用域](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Res-cond)
- [ES.7: 保持常用的和局部的名字尽量简短，而让非常用的和非局部的名字较长](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Res-name-length)
- [ES.8: 避免使用看起来相似的名字](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Res-name-similar)
- [ES.9: 避免 `ALL_CAPS` 式的名字](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Res-not-caps)
- [ES.10: 每条声明中（仅）声明一个名字](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Res-name-one)
- [ES.11: 使用 `auto` 来避免类型名字的多余重复](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Res-auto)
- [ES.12: 不要在嵌套作用域中重用名字](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Res-reuse)
- [ES.20: 坚持为对象进行初始化](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Res-always)
- [ES.21: 不要在确实需要使用变量（或常量）之前就引入它](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Res-introduce)
- [ES.22: 要等到获得了用以初始化变量的值之后才声明变量](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Res-init)
- [ES.23: 优先使用 `{}` 初始化式语法](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Res-list)
- [ES.24: 用 `unique_ptr` 来保存指针](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Res-unique)
- [ES.25: 应当将对象声明为 `const` 或 `constexpr`，除非后面需要修改其值](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Res-const)
- [ES.26: 不要用一个变量来达成两个不相关的目的](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Res-recycle)
- [ES.27: 使用 `std::array` 或 `stack_array` 作为栈上的数组](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Res-stack)
- [ES.28: 为复杂的初始化（尤其是 `const` 变量）使用 lambda](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Res-lambda-init)
- [ES.30: 不要用宏来操纵程序文本](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Res-macros)
- [ES.31: 不要用宏来作为常量或“函数”](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Res-macros2)
- [ES.32: 对所有的宏名采用 `ALL_CAPS` 命名方式](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Res-all_caps)
- [ES.33: 如果必须使用宏的话，请为之提供唯一的名字](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Res-macros)
- [ES.34: 不要定义（C 风格的）变参函数](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Res-ellipses)

表达式的规则：

- [ES.40: 避免复杂的表达式](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Res-complicated)
- [ES.41: 对运算符优先级不保准时应使用括号](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Res-parens)
- [ES.42: 保持单纯直接的指针使用方式](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Res-ptr)
- [ES.43: 避免带有未定义的求值顺序的表达式](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Res-order)
- [ES.44: 不要对函数参数求值顺序有依赖](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Res-order-fct)
- [ES.45: 避免“魔法常量”，采用符号化常量](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Res-magic)
- [ES.46: 避免窄化转换](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Res-narrowing)
- [ES.47: 使用 `nullptr` 而不是 `0` 或 `NULL`](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Res-nullptr)
- [ES.48: 避免强制转换](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Res-casts)
- [ES.49: 当必须使用强制转换时，使用具名的强制转换](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Res-casts-named)
- [ES.50: 不要强制掉 `const`](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Res-casts-const)
- [ES.55: 避免发生对范围检查的需要](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Res-range-checking)
- [ES.56: 仅在确实需要明确移动某个对象到别的作用域时才使用 `std::move()`](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Res-move)
- [ES.60: 避免在资源管理函数之外使用 `new` 和 `delete`](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Res-new)
- [ES.61: 用 `delete[\]` 删除数组，用 `delete` 删除非数组对象](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Res-del)
- [ES.62: 不要在不同的数组之间进行指针比较](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Res-arr2)
- [ES.63: 不要产生切片](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Res-slice)
- [ES.64: 使用 `T{e}` 写法来进行构造](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Res-construct)
- [ES.65: 不要解引用无效指针](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Res-deref)

语句的规则：

- [ES.70: 面临选择时，优先采用 `switch` 语句而不是 `if` 语句](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Res-switch-if)
- [ES.71: 面临选择时，优先采用范围式 `for` 语句而不是普通 `for` 语句](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Res-for-range)
- [ES.72: 当存在显然的循环变量时，优先采用 `for` 语句而不是 `while` 语句](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Res-for-while)
- [ES.73: 当没有显然的循环变量时，优先采用 `while` 语句而不是 `for` 语句](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Res-while-for)
- [ES.74: 优先在 `for` 语句的初始化部分中声明循环变量](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Res-for-init)
- [ES.75: 避免使用 `do` 语句](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Res-do)
- [ES.76: 避免 `goto`](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Res-goto)
- [ES.77: 尽量减少循环中使用的 `break` 和 `continue`](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Res-continue)
- [ES.78: 不要依靠 `switch` 语句中的隐含直落行为](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Res-break)
- [ES.79: `default`（仅）用于处理一般情况](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Res-default)
- [ES.84: 不要试图声明没有名字的局部变量](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Res-noname)
- [ES.85: 让空语句显著可见](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Res-empty)
- [ES.86: 避免在原生的 `for` 循环中修改循环控制变量](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Res-loop-counter)
- [ES.87: 请勿在条件上添加多余的 `==` 或 `!=`](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Res-if)

算术规则：

- [ES.100: 不要进行有符号和无符号混合运算](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Res-mix)
- [ES.101: 使用无符号类型进行位操作](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Res-unsigned)
- [ES.102: 使用有符号类型进行算术运算](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Res-signed)
- [ES.103: 避免上溢出](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Res-overflow)
- [ES.104: 避免下溢出](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Res-underflow)
- [ES.105: 避免除整数零](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Res-zero)
- [ES.106: 不要试图用 `unsigned` 来防止负数值](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Res-nonnegative)
- [ES.107: 不要对下标使用 `unsigned`，优先使用 `gsl::index`](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Res-subscripts)
...
**另请参见**: [SL.1](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rsl-lib), [ES.11](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Res-auto)
...
大多数测量都会称这个函数太长了，但其关键点是 `fn` 所使用的资源和 `is` 所持有的文件句柄 所持有的时间比其所需长太多了，而且可能在函数后面意外地出现对 `is` 和 `fn` 的使用。 这种情况下，把读取操作重构出来可能是一个好主意：
...
**另请参见**: [不要用一个变量来达成两个不相关的目的](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Res-recycle)
...
**讨论**：限制循环变量的作用域到循环体还相当有助于代码优化。认识到引入的变量仅在循环体中可以访问， 允许进行诸如外提（hoisting），强度消减，循环不变式代码外提等等优化。
...
我们有意让*总是进行初始化*规则比*对象在使用前必须设值*的语言规则更强。 后者是较为宽松的规则，虽然能够识别出技术上的 BUG，不过：
...
这段代码是无法简单重写为用初始化式来对 `i` 和 `j` 进行初始化的。 注意，对于带有默认构造函数的类型来说，试图延后初始化只会导致变为一次默认初始化之后跟着一次赋值的做法。 这种例子的一种更加流行的理由是“效率”，不过可以检查出是否出现“设置前使用”错误的编译器，同样可以消除任何多余的双重初始化。
...
如果除此之外 `make_related_widgets` 函数是多余的， 可以使用 lambda [ES.28](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Res-lambda-init) 来消除之：
...
**参见**: [ES.28](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Res-lambda-init)

##### 强制实施

- 标记出每个未初始化的变量。 不要对具有默认构造函数的自定义类型的变量进行标记。
- 检查未初始化的缓冲区是否在声明后*立即*进行了写入。 将未初始化变量作为一个非 `const` 的引用参数进行传递可以被假定为向变量进行的写入。
...
对于具有必要复杂性的初始化式，也包括 `const` 变量的初始化式，应当考虑使用 lambda 来表达它；参见 [ES.28](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Res-lambda-init)。
...
**参见**: [讨论](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#???)
...
查看变量是不是真的被改动过，若并非如此就进行标记。 不幸的是，也许不可能检测出某个非 `const` 是不是 *有意*要改动，还是仅仅是没被改动而已。
...
`a1` 的定义是合法的 C++ 而且一直都是。 存在大量的这类代码。 不过它是易错的，尤其当它的界并非局部时更是如此。 而且它也是一种“流行”的错误来源（缓冲区溢出，数组退化而成的指针，等等）。 而 `a2` 的定义符合 C 但不符合 C++，而且被认为存在安全性风险。
...
此规则也意味着不鼓励使用 `#` 进行字符串化和使用 `##` 进行连接。 照例，宏有一些“无害”的用途，但即使这些也会给工具带来麻烦， 例如自动完成器、静态分析器和调试器。 通常，使用花式宏的欲望是过于复杂的设计的标志。 另外，`＃` 和 `##` 促进了宏的定义和使用：
...
如果可能就应当避免使用宏：[ES.30](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Res-macros)，[ES.31](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Res-macros2)，以及 [ES.32](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Res-all_caps)。 然而，存在亿万行的代码中包含宏，以及一种使用并过度使用宏的长期传统。 如果你被迫使用宏的话，请使用长名字，而且应当带有唯一前缀（比如你的组织机构的名字）以减少冲突的可能性。
...
C++17 收紧了有关求值顺序的规则 （除了赋值中从右向左，以及函数实参求值顺序未指明外均为从左向右，[参见 ES.43](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Res-order)）， 但这并不影响复杂表达式很容易引起混乱的事实。
...
代之以使用 `gsl::span`。 指针[只应当指代单个对象](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Ri-array)。 指针算术是脆弱而易错的，是许多许多糟糕的 BUG 和安全漏洞的来源。 `span` 是一种用于访问数组对象的带有边界检查的安全类型。 以常量为下标来访问已知边界的数组，编译器可以进行验证。
...
本条规则属于[边界安全性剖面配置](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#SS-bounds)。
...
在 C++17 之前，其行为是未定义的。因此其行为可能是任何事（比如 `f(2, 2)`）。 自 C++17 起，中这段代码没有未定义行为，但仍未指定是哪个实参被首先求值。这个调用会是 `f(0, 1)` 或 `f(1, 0)`，但你不知道是哪个。
...
[类型剖面配置](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Pro-type-reinterpretcast) 禁止使用 `reinterpret_cast` 和 C 风格强制转换。

不要以强制转换为 `(void)` 来忽略 `[[nodiscard]]` 返回值。 当你有意要丢弃这种返回值时，应当首先深入思考这是不是确实是个好主意（通常，这个函数或者使用了 `[[nodiscard]]` 的返回类型的作者，当初确实是有充分理由的）。 要是你仍然觉得这样做是合适的，而且你的代码评审者也同意的话，使用 `std::ignore =` 来关闭这个警告，这既简单，可移植，也易于 grep。
...
- 对使用 `Type(value)` 的函数风格强制转换进行标记。应代之以使用不会发生窄化的 `Type{value}`。（参见 [ES.64](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Res-construct)。）
- 对指针类型之间的[同一强制转换](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Pro-type-identitycast)，若其中的源类型和目标类型相同(#Pro-type-identitycast)则进行标记。
- 对可以作为[隐式转换](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Pro-type-implicitpointercast)的显示指针强制转换进行标记。
...
- 对包括向 `void` 在内的所有 C 风格强制转换进行标记。
- 对使用 `Type(value)` 的函数风格强制转换进行标记。应代之以使用不会发生窄化的 `Type{value}`。（参见 [ES.64](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Res-construct)。）
- [类型剖面配置](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Pro-type-reinterpretcast)禁用了 `reinterpret_cast`。
- [类型剖面配置](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Pro-type-arithmeticcast)对于在算术类型之间使用 `static_cast` 时给出警告。
...
- 标记 `const_cast`。
- 本条规则属于[类型安全性剖面配置](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Pro-type-constcast)。
...
一次移动通常会遗留一个空对象（[C.64](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rc-move-semantic)），这可能令人意外甚至很危险，因此我们试图避免从左值进行移动（它们可能随后会被访问到）。
...
通常来说，`std::move()` 都用做某个 `&&` 形参的实参。 而这点之后，应当假定对象已经被移走（参见 [C.64](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rc-move-semantic)），而直到首次向它设置某个新值之前，请勿再次读取它的状态。
...
绝不要仅仅因为听说过“这样更加高效”就使用 `std::move()`。 通常来说，请不要相信那些没有支持数据的有关“效率”的断言。(???). 通常来说，请不要无理由地使代码复杂化。(??) 绝不要在 const 对象上 `std::move()`，它只会暗中将其转变成一个副本（参见 [Meyers15](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Meyers15) 的条款 23)。
...
**参见**: [R: 资源管理](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#S-resource)
...
这个例子不仅像上前一个例子一样违反了[禁止裸 `new` 规则](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Res-new)，它还有更多的问题。
...
这样的结果是无意义的，因为不会把中心和半径从 `c` 复制给 `s`。 针对这个的第一条防线是[将基类 `Shape` 定义为不允许这样做](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rc-copy-virtual)。
...
[避免强制转换](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Res-casts)（显式类型转换），如果必须要做的话[优先采用具名强制转换](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Res-casts-named)。
...
对象构造语法是最通用的[初始化式语法](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Res-list)。
...
本条规则是显然并且广为知晓的语言规则，但其可能难于遵守。 这需要良好的编码风格，程序库支持，以及静态分析来消除违反情况而不耗费大量开销。 这正是 [C++ 的类型和资源安全性模型](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Stroustrup15)中所讨论的主要部分。

**参见**：

- 使用 [RAII](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rr-raii) 以避免生存期问题。
- 使用 [unique_ptr](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rf-unique_ptr) 以避免生存期问题。
- 使用 [shared_ptr](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rf-shared_ptr) 以避免生存期问题。
- 当不可能出现 `nullptr` 时应使用[引用](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rf-ptr-ref)。
- 使用 [not_null](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rf-nullptr) 以尽早捕捉到预期外的 `nullptr`。
- 使用[边界剖面配置](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#SS-bounds)以避免范围错误。
...
本条规则属于[生存期安全性剖面配置](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#SS-lifetime)
...
参见 [ES.6](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Res-cond)
...
如果你确实要打断一个循环，使用 `break` 通常比使用诸如[修改循环变量](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Res-loop-counter)或 [`goto`](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Res-goto) 等其他方案更好：
...
这里声明了一个无名的 `lock_guard` 对象，它将在分号处立刻离开作用域。 这并不是一种少见的错误。 特别是，这个特别的例子会导致很难发觉的竞争条件。
...
根据定义，`if` 语句，`while` 语句，以及 `for` 语句中的条件，选择 `true` 或 `false` 的取值。 数值与 `0` 相比较，指针值与 `nullptr` 相比较。
...
不幸的是，C++ 使用有符号整数作为数组下标，而标准库使用无符号整数作为容器下标。 这妨碍了一致性。使用 `gsl::index` 来作为下标类型；[参见 ES.107](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Res-subscripts)。
...
使用 `gsl::index` 作为下标类型；[参见 ES.107](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Res-subscripts)。
...
- [Per.1: 请勿进行无理由的优化](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rper-reason)
- [Per.2: 请勿进行不成熟的优化](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rper-knuth)
- [Per.3: 请勿对非性能关键的代码进行优化](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rper-critical)
- [Per.4: 不能假定复杂代码一定比简单代码更快](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rper-simple)
- [Per.5: 不能假定低级代码一定比高级代码更快](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rper-low)
- [Per.6: 请勿不进行测量就作出性能评断](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rper-measure)
- [Per.7: 设计应当允许优化](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rper-efficiency)
- [Per.10: 依赖静态类型系统](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rper-type)
- [Per.11: 把计算从运行时转移到编译期](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rper-comp)
- [Per.12: 消除多余的别名](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rper-alias)
- [Per.13: 消除多余的间接](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rper-indirect)
- [Per.14: 最小化分配和回收的次数](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rper-alloc)
- [Per.15: 请勿在关键逻辑分支中进行分配](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rper-alloc0)
- [Per.16: 使用紧凑的数据结构](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rper-compact)
- [Per.17: 在时间关键的结构中应当先声明最常用的成员](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rper-struct)
- [Per.18: 空间即时间](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rper-space)
- [Per.19: 进行可预测的内存访问](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rper-access)
- [Per.30: 避免在关键路径中进行上下文切换](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rper-context)
...
换个角度来说：如果你的程序花费处理时间的 4% 来 计算 A 而花费 40% 的时间来计算 B，那对 A 的 50% 的改进 其影响只能和 B 的 5% 的改进相比。（如果你甚至不知道 A 或 B 到底花费了多少时间，参见 [Per.1](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rper-reason) 和 [Per.2](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rper-Knuth)。）
...
不成熟的优化被称为[一切罪恶之源](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rper-knuth)，但这并不是轻视性能的理由。 考虑如何使设计可以为改进而进行修正绝不是不成熟的，而性能改进则是一种常见的改进要求。 我们的目标是建立一组习惯，使缺省情况就能得到高效，可维护，且可优化的代码。 特别是，当你编写并非一次性实现细节的函数时，应当考虑

- 信息传递： 优先采用能够为后续的实现改进带来充分信息的简洁[接口](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#S-interfaces)。 要注意信息会通过我们所提供的接口来流入和流出一个实现。
- 紧凑的数据：默认情况[使用紧凑的数据](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rper-compact)，比如 `std::vector`，并[系统化地进行访问](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rper-access)。 如果你觉得需要一种有链接的结构的话，应尝试构造接口使这个结构不会被用户所看到。
- 函数的参数传递和返回： 对可改变和不可变的数据加以区分。 不要把资源管理的负担强加给用户。 不要把假性的间接强加给用户。 对通过接口传递信息采用[符合惯例的方式](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rf-conventional)； 不合惯例的，以及“优化过的”数据传递方式可能会严重影响后续的重新实现。
...
- 程序库： 使用带有良好接口的程序库。 当没有可用的程序库时，就构建自己的，并模仿一个好程序库的接口风格。 [标准库](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#sl-the-standard-library)是寻找模仿的一个好的第一来源。
...
有时候仅把最小数量的信息传递回来（如这里的 `true` 或 `false`）是足够的，但一个好的接口会 向调用方传递其所需的信息。因此，标准库还提供了
...
对效率的需求并不意味着对[底层代码](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rper-low)的需求。 高层代码并不必然缓慢或膨胀。
...
这条规则并不抵触[不要进行不成熟的优化](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rper-knuth)规则。 它对其进行了补充，鼓励程序员在有必要的时候，使后续的——适当并且成熟的——优化能够进行。
...
假定 `Scoped` 和 `On_heap` 均提供了兼容的用户接口。 这里我们在编译时计算出了最优的类型。 对于选择所要调用的最优函数，也有类似的技术。
...
我们经常想要我们的计算机同时运行许多任务（或者至少表现为同时运行它们）。 这有许多不同的原因（例如，需要仅用一个处理器等待许多事件，同时处理许多数据流，或者利用大量的硬件设施） 因此也由许多不同的用以表现并发和并行的基本设施。 我们这里将说明使用 ISO 标准 C++ 中用以表现基本并发和并行的设施的原则和规则。

线程是对并发和并行编程支持的机器级别的基础。 使用线程允许互不相关地运行程序的多个部分， 同时共享相同的内存。并发编程很麻烦， 因为在线程之间保护共享的数据说起来容易做起来难。 使现存的单线程代码可以并发执行， 可以通过策略性地添加 `std::async` 或 `std::thread` 这样简单做到， 也可能需要进行完全的重写，这依赖于原始代码是否是以线程友好 的方式编写的。

本文档中的并发/并行规则的设计有三个 目标：

- 有助于编写可以被修改为能够在线程环境中使用 的代码。
- 展示使用标准库所提供的线程原语的简洁， 安全的方式。
- 对当并发和并行无法提供所需要的性能增益时应当如何做 提供指导。

同样重要的一点，是要注意 C++ 的并发仍然是未完成的工作。 C++11 引入了许多核心并发原语，C++14 和 C++17 对它们进行了改进， 而且在使 C++ 编写并发程序更加简单的方面 仍有许多关注。我们预计这里的一些与程序库相关 的指导方针会随着时间有大量的改动。

这一部分需要大量的工作（显然如此）。 请注意我们的规则是从相对非专家们入手的。 真正的专家们还请稍等； 我们欢迎贡献者， 但请为正在努力使他们的并发程序正确且高效的大多数程序员着想。

并发和并行规则概览：

- [CP.1: 假定你的代码将作为多线程程序的一部分而运行](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rconc-multi)
- [CP.2: 避免数据竞争](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rconc-races)
- [CP.3: 最小化可写数据的明确共享](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rconc-data)
- [CP.4: 以任务而不是线程的角度思考](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rconc-task)
- [CP.8: 不要为同步而使用 `volatile`](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rconc-volatile)
- [CP.9: 只要可行，就使用工具对并发代码进行验证](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rconc-tools)

**参见**：

- [CP.con: 并发](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#SScp-con)
- [CP.coro: 协程](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#SScp-coro)
- [CP.par: 并行](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#SScp-par)
- [CP.mess: 消息传递](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#SScp-mess)
- [CP.vec: 向量化](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#SScp-vec)
- [CP.free: 无锁编程](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#SScp-free)
- [CP.etc: 其他并发规则](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#SScp-etc)
...
这里的缓存作为 `ComputationCache` 对象的成员数据保存，而非共享的静态状态。 这项重构本质上是将关注点委派给了调用方： 单线程程序可能仍会选择采用一个全局的 `ComputationCache` 对象， 而多线程程序可能会为每个线程准备一个 `ComputationCache`，或者为任意定义的“上下文”每个准备一个。 重构后的函数不再试图管理 `cached_x` 的分配。 这方面可以看做是对单一职责原则（SRP）的一次应用。

在这个特定的例子中，为线程安全性进行的重构同样改进了其在单线程程序中的可用性。 不难想象，某个单线程程序可能需要在程序的不同部分中使用两个 `ComputationCache` 实例， 并且它们并不会覆盖掉互相的缓冲数据。

还有其他的一些方法，可以为针对标准多线程环境（就是唯一的并发形式是 `std::thread` 的环境）编写的 代码添加线程安全性：
...
简而言之，当两个线程并发（未同步）地访问同一个对象，且至少一方为写入方（实施某个非 `const` 操作）时，就会出现数据竞争。 有关如何正确使用同步来消除数据竞争的更多信息，请求教于一本有关并发的优秀书籍（参见 [认真学习文献](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rconc-literature)）。
...
不可变数据可以安全并高效地共享。 无须对其进行锁定：不可能在常量上发生数据竞争。 另请参见[CP.mess: 消息传递](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#SScp-mess)和[CP.31: 优先采用按值传递](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rconc-data-by-value)。
...
[`volatile` 的（罕见）恰当用法](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rconc-volatile2)
...
- 静态强制实施工具：[clang](http://clang.llvm.org/docs/ThreadSafetyAnalysis.html) 和一些老版本的 [GCC](https://gcc.gnu.org/wiki/ThreadSafetyAnnotation) 都提供了一些针对线程安全性性质的静态代码标注。 统一地采用这项技术可以将许多种类的线程安全性错误变为编译时的错误。 这些代码标注一般都是局部的（将某个特定数据成员标记为又一个特定的互斥体进行防护）， 且一般都易于学习使用。但与许多的静态工具一样，它经常会造成漏报； 这些情况应当被发现但却被其忽略了。
- 动态强制实施工具：Clang 的 [Thread Sanitizer](http://clang.llvm.org/docs/ThreadSanitizer.html)（即 TSAN） 是动态工具的一个强有力的例子：它改变你的程序的构建和执行，向其中添加对内存访问的簿记工作， 精确地识别你的二进制程序的一次特定执行中发生的数据竞争。 使用它的代价在内存上（多数情况为五到十倍），也拖慢 CPU（二到二十倍）。 像这样的动态工具，在集成测试，金丝雀推送，以及在多个线程上操作的单元测试上实施是最好的。 它是与工作负载相关的：一旦 TSAN 识别了一个问题，那它就是实际发生的数据竞争， 但它只能识别出一次特定的执行之中发生的竞争。
...
- 有关并行算法，参见[并行](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#SScp-par)。
- 有关不使用明确共享的任务间，通信参见[消息传递](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#SScp-mess)。
- 有关向量并行代码，参见[向量化](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#SScp-vec)。
- 有关无锁编程，参见[无锁](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#SScp-free)。

并发规则概览：

- [CP.20: 使用 RAII，绝不使用普通的 `lock()`/`unlock()`](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rconc-raii)
- [CP.21: 用 `std::lock()` 或 `std::scoped_lock` 来获得多个 `mutex`](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rconc-lock)
- [CP.22: 绝不在持有锁的时候调用未知的代码（比如回调）](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rconc-unknown)
- [CP.23: 把联结的 `thread` 看作是有作用域的容器](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rconc-join)
- [CP.24: 把 `thread` 看作是全局的容器](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rconc-detach)
- [CP.25: 优先采用 `gsl::joining_thread` 而不是 `std::thread`](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rconc-joining_thread)
- [CP.26: 不要 `detach()` 线程](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rconc-detached_thread)
- [CP.31: 少量数据在线程之间按值传递，而不是通过引用或指针传递](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rconc-data-by-value)
- [CP.32: 用 `shared_ptr` 在无关的 `thread` 之间共享所有权](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rconc-shared)
- [CP.40: 最小化上下文切换](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rconc-switch)
- [CP.41: 最小化线程的创建和销毁](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rconc-create)
- [CP.42: 不要无条件地 `wait`](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rconc-wait)
- [CP.43: 最小化临界区的时间耗费](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rconc-time)
- [CP.44: 记得为 `lock_guard` 和 `unique_lock` 命名](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rconc-name)
- [CP.50: `mutex` 要和其所护卫的数据一起定义。一旦可能就使用 `synchronized_value`](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rconc-mutex)
- ??? 何时使用 spinlock
- ??? 何时使用 `try_lock()`
...
如果你[不 `detach()`](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rconc-detached_thread) 并[使用 `gsl::joining_tread`](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rconc-joining_thread) 的话，本条规则是多余的。 不过，将代码转化为遵循这些指导方针可能很困难，而对于第三方库来说更是不可能的。 这些情况下，这条规则对于生存期安全性和类型安全性来说就是必要的了。
...
使“不死线程”成为全局的，将其放入外围作用域中，或者放入自由存储中，而不要 `detach()` 它们。 [不要 `detach`](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rconc-detached_thread)。
...
- 建议当其脱离时使其[“外放所有权”](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rconc-detached_thread)到某个外围作用域中。
...
这里我们持有的锁定比所需的要长： 我们不应该在必须锁定之前就获取锁定，而且应当在开始清理之前将其释放掉。 可以将其重写为：
...
但这样损害了安全性并且违反了[使用 RAII](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rconc-raii) 规则。 我们可以为临界区添加语句块：
...
这个貌似足够有效，但其实并非如此。在 (A) 点，`m1` 是一个 默认构造的局部 `unique_lock`，它隐藏了全局的 `::m1`（且并未锁定它）。 在 (B) 点，构造了一个无名 `lock_guard` 临时对象并锁定了 `::m2`， 但它立即离开作用域并再次解锁了 `::m2`。 函数 `f()` 的余下部分中并没有锁定任何互斥体。
...
使用 `synchronized_value<T>` 保证了数据都带有互斥体，并且当访问数据时锁定正确的互斥体。 参见向某个未来的 TS 或 C++ 标准的修订版添加 `synchronized_value` [WG21 提案](http://wg21.link/p0290)。
...
- [CP.51: 不要使用作为协程的有俘获 lambda 表达式](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rcoro-capture)
- [CP.52: 不要在持有锁或其它同步原语时跨越挂起点](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rcoro-locks)
- [CP.53: 协程的形参不能按引用传递](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rcoro-reference-parameters)
...
输出参数也有这种危险。[F.20: 对于“输出（out）”值，采用返回值优先于输出参数](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rf-out) 不建议使用输出参数。协程应当完全避免输出参数。
...
标准库的设施是相当底层的，关注于使用 `thread`，`mutex`，`atomic` 等类型的贴近硬件的关键编程。 大多数人都不应该在这种层次上工作：它是易错的，而且开发很慢。 如果可能的话，应当使用高层次的设施：消息程序库，并行算法，以及向量化。 这一部分关注如何传递消息，以使程序员不必进行显式的同步。

消息传递规则概览：

- [CP.60: 使用 `future` 从并发任务返回值](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rconc-future)
- [CP.61: 使用 `async()` 来产生并发任务](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rconc-async)
- 消息队列
- 消息程序库
...
[R.12](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rr-immediate-alloc) 告诉我们要避免原始所有权指针，与此相似， 我们也要尽可能避免原始线程和原始承诺（promise）。应使用诸如 `std::async` 之类的工厂函数， 它将处理线程的产生和重用，而不会讲原始线程暴露给你自己的代码。
...
不幸的是，`async()` 并不完美。比如说，它并不使用线程池， 这意味着它可能会因为资源耗尽而失败，而不会将你的任务放入队列以便随后执行。 不过，即便你不能用 `std::async`，你也应当优先编写自己的返回 `future` 的工厂函数，而非使用原始承诺。
...
这个例子展示一种方法，可以让你在当 `std::async` 自身 在产品中不可接受的情形中，模仿 `std::async` 所设立的 一般模式的方法。
...
向量化是一种在不引入显式同步时并发地执行多个任务的技术。 它会并行地把某个操作实施与某个数据结构（向量，数组，等等）的各个元素之上。 向量化引人关注的性质在于它通常不需要对程序进行非局部的改动。 不过，向量化只对简单的数据结构和特别针对它所构造的算法能够得到最好的工作效果。
...
使用 `mutex` 和 `condition_variable` 进行同步相对来说很昂贵。 而且可能导致死锁。 为了性能并消除死锁的可能性，有时候我们不得不使用麻烦的底层“无锁”设施， 它们依赖于对内存短暂地获得互斥（“原子性”）访问。 无锁编程也被用于实现如 `thread` 和 `mutex` 这样的高层并发机制。

无锁编程规则概览：

- [CP.100: 除非绝对必要，请勿使用无锁编程](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rconc-lockfree)
- [CP.101: 不要信任你的硬件-编译器组合](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rconc-distrust)
- [CP.102: 仔细研究文献](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rconc-literature)
- 如何/何时使用原子
- 避免饥饿
- 使用无锁数据结构而不是手工构造的专门的无锁访问
- [CP.110: 不要为初始化编写你自己的双检查锁定](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rconc-double)
- [CP.111: 当确实需要双检查锁定时应当采用惯用的模式](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rconc-double-pattern)
- 如何/何时进行比较并交换（CAS）
...
请找出这里的 BUG。 通过测试找到它是非常困难的。 请阅读有关 ABA 问题的材料。

##### 例外

[原子变量](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#???)可以简单并安全地使用，只要你所用的是顺序一致性内存模型（`memory_order_seq_cst`），而这是默认情况。
...
指令重排（静态和动态的）会让我们很难有效在这个层次上进行思考（尤其当你使用宽松的内存模型的时候）。 经验，（半）形式化的模型以及模型检查可以提供帮助。 测试——通常需要极端程度——是基础。 “不要飞得太靠近太阳。”
...
双检查锁定是很容易被搞乱的。如果确实需要编写自己的双检查锁定，而不顾规则 [CP.110: 不要为初始化编写你自己的双检查锁定](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rconc-double)和规则 [CP.100: 除非绝对必要，请勿使用无锁编程](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rconc-lockfree)，那么应当采用惯用的模式。

使用双检查锁定模式而不违反[CP.110: 不要为初始化编写你自己的双检查锁定](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rconc-double)的情形，出现于当某个非线程安全的动作既困难也罕见，并且存在某个快速且线程安全的测试可以用于保证该动作并不需要实施的情况，但反过来的情况则无法保证。

##### 示例，不好

使用 `volatile` 并不能使得第一个检查线程安全，另见[CP.200: `volatile` 仅用于和非 C++ 内存进行通信](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rconc-volatile2)
...
- [CP.200: `volatile` 仅用于和非 C++ 内存进行通信](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rconc-volatile2)
- [CP.201: ??? 信号](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rconc-signal)
...
这说明了一个被某个时钟电路不断更新的寄存器。 `clock` 为 `volatile` 是因为其值将会在没有使用它的 C++ 程序的任何动作下被改变。 例如，两次读取 `clock` 经常会产生两个不同的值，因此优化器最好不要将下面代码中的第二个读取操作优化掉：
...
`volatile` 局部变量几乎都是错误的——既然是短暂的，它们如何才能共享给其他语言或硬件呢？ 因为相同的理由，这几乎同样强有力地适用于数据成员。
...
与其他一些语言不同，C++ 中的 `volatile` [和同步没有任何关系](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rconc-volatile)。
...
- [E.1: 在设计中尽早开发错误处理策略](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Re-design)
- [E.2: 通过抛出异常来明示函数无法完成其所赋予的任务](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Re-throw)
- [E.3: 仅使用异常来进行错误处理](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Re-errors)
- [E.4: 围绕不变式来设计错误处理策略](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Re-design-invariants)
- [E.5: 让构造函数建立不变式，若其无法做到则抛出异常](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Re-invariant)
- [E.6: 使用 RAII 来避免泄漏](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Re-raii)
- [E.7: 明示前条件](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Re-precondition)
- [E.8: 明示后条件](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Re-postcondition)
- [E.12: 当函数不可能或不能接受以 `throw` 来退出时，使用 `noexcept`](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Re-noexcept)
- [E.13: 不要在作为某个对象的直接所有者时抛出异常](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Re-never-throw)
- [E.14: 应当使用为目的所设计的自定义类型（而不是内建类型）作为异常](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Re-exception-types)
- [E.15: 按值抛出并按引用捕获类型层次中的异常](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Re-exception-ref)
- [E.16: 析构函数，回收函数，`swap`，以及异常类型的复制/移动构造决不能失败](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Re-never-fail)
- [E.17: 不要试图在每个函数中捕获每个异常](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Re-not-always)
- [E.18: 最小化对 `try`/`catch` 的显式使用](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Re-catch)
- [E.19: 当没有合适的资源包装时，使用 `final_action` 对象来表达清理动作](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Re-finally)
- [E.25: 当不能抛出异常时，模拟 RAII 来进行资源管理](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Re-no-throw-raii)
- [E.26: 当不能抛出异常时，考虑采取快速失败](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Re-no-throw-crash)
- [E.27: 当不能抛出异常时，系统化地使用错误代码](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Re-no-throw-codes)
- [E.28: 避免基于全局状态（比如 `errno`）的错误处理](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Re-no-throw)
- [E.30: 请勿使用异常说明](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Re-specifications)
- [E.31: 恰当地对 `catch` 子句排序](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Re_catch)
...
这里，`vector` 和 `string` 的构造函数可能无法为其元素分配足够的内存，`vector` 的构造函数可能无法复制其初始化式列表中的 `Thing`，而 `File_handle` 可能无法打开所需的文件。 这些情况中，它们都会抛出异常来让 `use()` 的调用者来处理。 如果 `use()` 可以处理这种构造 `bar` 的故障的话，它可以用 `try`/`catch` 来控制。 这些情况下，`Foo` 的构造函数都会在把控制传递给试图创建 `Foo` 的任何代码之前恰当地销毁以及构造的内存。 注意它并不存在可以包含错误代码的返回值。
...
人们通常说异常应当用于表明意外的事件和故障。 不过这里面存在一点循环，“什么是意外的？” 例如：

- 无法满足的前条件
- 无法构造对象的构造函数（无法建立类的[不变式](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rc-struct)）
- 越界错误（比如 `v[v.size()] = 7`）
- 无法获得资源（比如网络未连接）

相较而言，终止某个普通的循环则不是意外的。 除非这个循环本应当是无限循环，否则其终止就是正常且符合预期的。
...
**参见**: [讨论](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Sd-noexcept)

##### 注解

在你决定你无法负担或者不喜欢基于异常的错误处理之前，请看一看[替代方案](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Re-no-throw-raii)； 它们各自都有自己的复杂性和问题。 同样地，只要可能的话，就应该进行测量之后再发表有关效率的言论。
...
这种代码要比显然的替代方式更加复杂，而且极可能运行慢得多。 在 `vector` 中寻找一个值是没什么意外情况的。

##### 强制实施

可能应该是启发式措施。 查找从 `catch` 子句“漏掉”的异常值。
...
[不变式](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rc-struct)是对象的成员的逻辑条件，构造函数必须进行建立，且为公开的成员函数所假定。
...
类不变式——这里以代码注释说明——是由构造函数建立的。 当 `new` 无法分配所需的内存时将抛出异常。 各运算符，尤其是下标运算符，都是依赖于这个不变式的。

**参见**: [当构造函数无法构造有效对象时，应当抛出异常](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rc-throw)
...
当没有合适的资源包装，且定义一个适当的 RAII 对象/包装由于某种原因不可行时， 万不得已，可以使用 [`final_action` 对象](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Re-finally)来表达清理动作。

##### 注解

但是当我们所编写的程序不能使用异常时应当怎么办呢？ 首先应当质疑这项假设；到处都有许多反异常的错误认识。 据我们所知，只有少量正当理由：
...
以上原因中只有第一条才是基础问题，因此一旦可能的话，还是要用异常来实现 RAII，或者设计你的 RAII 对象永不失败。 当无法使用异常时，可以模拟 RAII。 就是说，系统化地在对象构造之后检查其有效性，并且仍然在析构函数中释放所有的资源。 一种策略是为每个资源包装添加一个 `valid()` 操作：
...
显然这样做增加了代码大小，不允许隐式的“异常”（`valid()` 检查）传播，而且 `valid()` 检查可能被忘掉。 优先采用异常。

**参见**: [`noexcept` 的用法](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Re-noexcept)
...
**参见**: [前条件规则](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Ri-pre)
...
**参见**: [后条件规则](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Ri-post)
...
请勿使用传统的[异常说明](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Re-specifications)。

##### 参见

[讨论](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Sd-noexcept)。
...
如果有需要清理的某个局部“东西”，但其并未表示为带有析构函数的对象，则这样的清理 也必须在 `throw` 之前完成。 有时候，[`finally()`](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Re-finally) 可以把这种不系统的清理变得更加可管理一些。
...
大多数处理器并不会改动异常，一般情况下我们都会[建议使用 `const`](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Res-const)。
...
重新抛出已捕获的异常应当使用 `throw;` 而非 `throw e;`。使用 `throw e;` 将会抛出 `e` 的一个新副本（并于异常被 `catch (const std::exception& e)` 捕获时切片成静态类型 `std::exception`），而并非重新抛出原来的 `std::runtime_error` 类型的异常。（但请关注[请勿试图在每个函数中捕获所有的异常](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Re-not-always)，以及[尽可能减少 `try`/`catch` 的显式使用](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Re-catch)。)
...
许多人都曾试图编写违反这条规则的可靠代码，比如当网络连接“拒绝关闭”的情形。 尽我们所知，没人曾找到一个做到这点的通用方案。 虽然偶尔对于非常特殊的例子，你可以通过设置某个状态以便进行将来的清理的方式绕过它。 比如说，我们可能将一个不打算关闭的 socket 放入一个“故障 socket”列表之中， 让其被某个定期的系统状态清理所检查处理。 我们见过的每个这种例子都是易错的，专门的，而且通常有 BUG。
...
- 而且，[应该使移动操作为 `noexcept`](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rc-move-noexcept)。
...
**参见**: [讨论](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Sd-never-fail)

### E.17: 不要试图在每个函数中捕获每个异常

##### 理由

如果函数无法对异常进行有意义的恢复动作，其捕获这个异常就导致复杂性和浪费。 要让异常传播直到遇到一个可以处理它的函数。 要用 [RAII](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Re-raii) 来处理栈回溯路径上的清理操作。
...
这段代码很混乱。 可能在 `try` 块中的裸指针上发生泄漏。 不是所有的异常都被处理了。 `delete` 一个构造失败的对象几乎肯定是一个错误。 更好的是：
...
- 合适的资源包装以及 [RAII](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Re-raii)
- [`finally`](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Re-finally)
...
[GSL](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#gsl-guidelines-support-library) 提供的 `finally` 要比 `try`/`catch` 更不啰嗦且难于搞错。
...
`finally` 没有 `try`/`catch` 那样混乱，但它仍然比较专门化。 优先采用[适当的资源管理对象](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Re-raii)。 万不得已考虑使用 `finally`。

##### 注解

相对于老式的 [`goto exit;` 技巧](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Re-no-throw-codes)来说，使用 `finally` 是处理并非系统化的资源管理中的清理工作的 更加系统化并且相当简洁的方案。
...
即便没有异常，[RAII](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Re-raii) 通常仍然是最佳且最系统化的处理资源的方式。

##### 注解

使用异常进行错误处理，是 C++ 中唯一完整且系统化的处理非局部错误的方式。 特别是，非侵入地对对象构造的失败进行报告需要使用异常。 以无法被忽略的方式报告错误需要使用异常。 如果无法使用异常，应当尽你所能模拟它们的使用。

大量对异常的惧怕是被误导的。 当用在并非充斥指针和复杂控制结构的代码中的违例情形时， 异常处理几乎总是（在时间和空间上）可以负担，且几乎总会导致更好的代码。 当然这假定存在一个优良的异常处理机制实现，而这并非在所有系统上都存在。 存在另一些情况并不适用于上述问题，但由于其他原因而无法使用异常。 一个例子是一些硬实时系统：必须在固定的时间之内完成操作并得到错误或正确的响应。 在没有适当的时间评估工具的条件下，很难对异常作出保证。 这样的系统（比如飞控系统）通常也会禁止使用动态（堆）内存。

因此，对于错误处理的主要指导方针还是“使用异常和 [RAII](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Re-raii)。” 本节所处理的情况是，要么你没有高效的异常实现， 或者要面对大量的老式代码 （比如说，大量的指针，不明确定义的所有权，以及大量的不系统化的基于错误代码检查的错误处理） 而且向其引入简单且系统化的错误处理的做法不可行。

在宣称不能使用异常或者抱怨它们成本过高之前，应当考虑一下使用[错误代码](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Re-no-throw-codes)的例子。 请考虑使用错误代码的成本和复杂性。 如果你担心性能的话，请进行测量。
...
当这个 `g` 并未正确构造时，`func` 将以一个异常退出。 当无法抛出异常时，我们可以通过向 `Gadget` 添加 `valid()` 成员函数来模拟 RAII 风格的资源包装：
...
**参见**: [讨论](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Sd-???)
...
**参见**：[模拟 RAII](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Re-no-throw-raii)

##### 注解

当你无法系统化地进行错误处理时，考虑以“程序崩溃”作为对任何无法局部处理的错误的回应。 就是说，如果你无法在检测到错误的函数的上下文中处理它，则调用 `abort()`，`quick_exit()`， 或者相似的某个将触发某种系统重启的函数。

在具有大量进程和/或大量计算机的系统中，你总要预计到并处理这些关键程序崩溃， 比如说源于硬件故障而引发。 这种情况下，“程序崩溃”只不过把错误处理留给了系统的下一个层次。
...
通常，返回错误指示意味着返回两个值：其结果以及一个错误指示。 错误指示可以是对象的一部分，例如对象可以带有 `valid()` 指示， 也可以返回一对值。
...
这种方案符合[模拟 RAII 资源管理](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Re-no-throw-raii)。 `valid()` 函数可以返回一个 `error_indicator`（比如说 `error_indicator` 枚举的某个成员）。

##### 示例

要是我们无法或者不想改动 `Gadget` 类型呢？ 这种情况下，我们只能返回一对值。 例如：
...
可见，`std::pair` 是一种可能的返回类型。 某些人则更喜欢专门的类型。 例如：
...
模拟 RAII 可能不那么简单，尤其是在带有多个资源和多种可能错误的函数之中。 一种较为常见的技巧是把清理都集中到函数末尾以避免重复（注意这里本不必为 `g2` 增加一层作用域，但是编译 `goto` 版本却需要它）：
...
函数越大，这种技巧就越有吸引力。 `finally` 可以[略微缓解这个问题](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Re-finally)。 而且，程序变得越大，系统化地采用一中基于错误指示的错误处理策略就越加困难。

我们[优先采用基于异常的错误处理](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Re-throw)，并建议[保持函数短小](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rf-single)。

**参见**: [讨论](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Sd-???)

**参见**: [返回多个值](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rf-out-multi)
...
常量是不会出现竞争条件的。 当大量的对象不会改变它们的值时，对程序进行推理将变得更容易。 承诺“不改动”作为参数所传递对象的接口，极大地提升了可读性。

常量规则概览：

- [Con.1: 缺省情况下，对象应当是不可变的](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rconst-immutable)
- [Con.2: 缺省情况下，成员函数应当为 `const`](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rconst-fct)
- [Con.3: 缺省情况下，应当传递指向 `const` 对象的指针或引用](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rconst-ref)
- [Con.4: 构造之后不再改变其值的对象应当以 `const` 来定义](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rconst-const)
- [Con.5: 以 `constexpr` 来定义可以在编译期计算的值](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rconst-constexpr)
...
按值返回，且移动比复制更廉价的局部变量，不应声明为 `const`， 因为这将强迫进行不必要的复制。
...
按值传递的函数参数很少被改动，但也很少被声明为 `const`。 为了避免造成混淆和大量的误报，不要对函数参数实施这条规则。。
...
- “强制掉 `const`”（[最好避免这样做](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Res-casts-const)）；
- 提供包装函数。
...
另一种说法是 `const` 特性不会传递。 通过 `const` 成员函数改动 `mutable` 成员的值和通过非 `const` 指针来访问的对象的值 是有可能的。 由类负责确保这样的改动仅当根据其语义（不变式）对于其用户有意义时 才会发生。

**参见**：[PImpl](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Ri-pimpl)
...
[请勿强制掉 `const`](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Res-casts-const)。
...
泛型编程是使用以类型、值和算法进行参数化的类型和算法所进行的编程。 在 C++ 中，泛型编程是以“模板”语言机制来提供支持的。

泛型函数的参数是以针对参数类型及其所涉及的值的规定的集合来刻画的。 在 C++ 中，这些规定是以被称为概念（concept）的编译期谓词来表达的。

模板还可用于进行元编程；亦即由编译期代码所组成的程序。

泛型编程的中心是“概念”；亦即以编译时谓词表现的对于模板参数的要求。 C++20 已经将“概念”标准化了，不过是在 GCC 6.1 中以一种略有不同的语法首次提供的。

模板使用的规则概览：

- [T.1: 用模板来提升代码的抽象层次](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rt-raise)
- [T.2: 用模板来表达适用于许多参数类型的算法](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rt-algo)
- [T.3: 用模板来表达容器和范围](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rt-cont)
- [T.4: 用模板来表达对语法树的操作](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rt-expr)
- [T.5: 结合泛型和面向对象技术来增强它们的能力，而不是它们的成本](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rt-generic-oo)

概念使用的规则概览：

- [T.10: 为所有模板实参指明概念](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rt-concepts)
- [T.11: 尽可能采用标准概念](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rt-std-concepts)
- [T.12: 对于局部变量，优先采用概念名而不是 `auto`](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rt-auto)
- [T.13: 对于简单的单类型参数概念，优先采用简写形式](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rt-shorthand)
- ???

概念定义的规则概览：

- [T.20: 避免没有有意义的语义的“概念”](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rt-low)
- [T.21: 为概念提出一组完整的操作要求](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rt-complete)
- [T.22: 为概念指明公理](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rt-axiom)
- [T.23: 通过添加新的使用模式，从更一般情形的概念中区分出提炼后的概念](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rt-refine)
- [T.24: 用标签类或特征类来区分仅在语义上存在差别的概念](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rt-tag)
- [T.25: 避免互补性的约束](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rt-not)
- [T.26: 优先采用使用模式而不是简单的语法来定义概念](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rt-use)
- [T.30: 节制地采用概念求反（`!C`）来表示微小差异](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rt-???)
- [T.31: 节制地采用概念析取（disjunction）（`C1 || C2`）来表示备选项](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rt-???)
- ???

模板接口的规则概览：

- [T.40: 使用函数对象向算法传递操作](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rt-fo)
- [T.41: 在模板的概念上仅提出基本的性质要求](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rt-essential)
- [T.42: 使用模板别名来简化写法并隐藏实现细节](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rt-alias)
- [T.43: 优先使用 `using` 而不是 `typedef` 来定义别名](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rt-using)
- [T.44: （如果可行）使用函数模板来对类模板的参数类型进行推断](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rt-deduce)
- [T.46: 要求模板参数至少是半正规的](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rt-regular)
- [T.47: 避免用常用名字命名高度可见的无约束模板](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rt-visible)
- [T.48: 如果你的编译器不支持概念的话，可以用 `enable_if` 来模拟](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rt-concept-def)
- [T.49: 尽可能避免类型擦除](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rt-erasure)

模板定义的规则概览：

- [T.60: 最小化模板的上下文依赖性](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rt-depend)
- [T.61: 请勿对成员进行过度参数化（SCARY）](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rt-scary)
- [T.62: 将无依赖的类模板成员置于一个非模板基类之中](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rt-nondependent)
- [T.64: 用特化来提供类模板的其他实现](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rt-specialization)
- [T.65: 用标签分派来提供函数的其他实现](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rt-tag-dispatch)
- [T.67: 用特化来提供不规则类型的其他实现](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rt-specialization2)
- [T.68: 在模板中用 `{}` 而不是 `()` 以避免歧义](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rt-cast)
- [T.69: 在模板中，请勿进行未限定的非成员函数调用，除非有意将之作为定制点](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rt-customization)

模板和类型层次的规则概览：

- [T.80: 请勿不成熟地对类层次进行模板化](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rt-hier)
- [T.81: 请勿混合类层次和数组](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rt-array) // ??? 放在“类型层次”部分
- [T.82: 当不想要虚函数时，可以将类层次线性化](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rt-linear)
- [T.83: 请勿声明虚的成员函数模板](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rt-virtual)
- [T.84: 使用非模板的核心实现来提供 ABI 稳定的接口](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rt-abi)
- [T.??: ????](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rt-???)

变参模板的规则概览：

- [T.100: 当需要可以接受可变数量的多种类型参数的函数时，使用变参模板](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rt-variadic)
- [T.101: ??? 如何向变参模板传递参数 ???](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rt-variadic-pass)
- [T.102: ??? 如何处理变参模板的参数 ???](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rt-variadic-process)
- [T.103: 请勿对同质参数列表使用变参模板](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rt-variadic-not)
- [T.??: ????](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rt-???)

元编程的规则概览：

- [T.120: 仅当确实需要时才使用模板元编程](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rt-metameta)
- [T.121: 模板元编程主要用于模拟概念机制](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rt-emulate)
- [T.122: 用模板（通常为模板别名）来在编译期进行类型运算](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rt-tmp)
- [T.123: 用 `constexpr` 函数来在编译期进行值运算](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rt-fct)
- [T.124: 优先使用标准库的模板元编程设施](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rt-std-tmp)
- [T.125: 当需要标准库之外的模板元编程设施时，使用某个现存程序库](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rt-lib)
- [T.??: ????](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rt-???)

其他模板规则概览：

- [T.140: 若操作可被重用，则应为其命名](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rt-name)
- [T.141: 当仅在一个地方需要一个简单的函数对象时，使用无名的 lambda](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rt-lambda)
- [T.142: 使用模板变量以简化写法](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rt-var)
- [T.143: 请勿编写并非有意非泛型的代码](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rt-nongeneric)
- [T.144: 请勿特化函数模板](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rt-specialize-function)
- [T.150: 用 `static_assert` 来检查类是否与概念相符](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rt-check-class)
- [T.??: ????](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rt-???)
...
由于假设 `Incrementable` 并不支持 `+`，而 `Simple_number` 也不支持 `+=`，我们对 `sum1` 和 `sum2` 的实现者过度约束了。 而且，这种情况下也错过了一次通用化的机会。
...
通过假定 `Arithmetic` 同时要求 `+` 和 `+=`，我们约束 `sum` 的使用者来提供完整的算术类型。 这并非是最小的要求，但它为算法的实现者更多所需的自由度，并确保了任何 `Arithmetic` 类型 都可被用于各种各样的算法。
...
除非确实需要多于一种模板参数类型，否则请不要使用模板。 请勿过度抽象。
...
**例外**: 当需要 ABI 稳定的接口时，也许你不得不提供一个基本实现，在基于它来呈现（类型安全的）目标。 参见[稳定的基类](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rt-abi)。
...
动态有助于静态：提供通用的，便利的，静态绑定的接口，但内部进行动态派发，这样就可以提供统一的对象布局。 这样的例子包括如 `std::shared_ptr` 的删除器的类型擦除。（不过[请勿过度使用类型擦除](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rt-erasure)。）
...
- [T.10: 为所有模板实参指明概念](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rt-concepts)
- [T.11: 尽可能采用标准概念](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rt-std-concepts)
- [T.12: 优先采用概念名而不是 `auto`](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rt-auto)
- [T.13: 对于简单的单类型参数概念，优先采用简写形式](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rt-shorthand)
- ???
...
普通的 `typename`（或 `auto`）是受最少约束的概念。 应当仅在只能假定“这是一个类型”的罕见情况中使用它们。 通常，这只会在当我们（用模板元编程代码）操作纯粹的表达式树，并推迟进行类型检查时才会需要。
...
“标准”概念（即由 [GSL](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#gsl-guidelines-support-library) 和 ISO 标准自身所提供的概念)， 避免了我们思考自己的概念，它们比我们匆忙中能够想出来的要好得多，而且还提升了互操作性。
...
这个 `Ordered_container` 貌似相当合理，但它和标准库中的 `sortable` 概念非常相似。 它是更好？更正确？它真的精确地反映了标准对于 `sort` 的要求吗？ 直接使用 `sortable` 则更好而且更简单：
...
定义恰当的概念并不简单。 概念是用于表现应用领域中的基本概念的（正如其名“概念”）。 只是把用在某个具体的类或算法的参数上的一组语法约束聚在一起，并不是概念所设计的用法， 也无法获得这个机制的全部好处。
...
也许拼接是有意进行的。不过更可能的是一种意外。而对减法进行同等的定义则会导致可接受类型的集合的非常不同。 这个 `Addable` 违反了加法应当可交换的数学法则：`a+b == b+a`。
...
这是对一般性规则[必须让概念有语义上的意义](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rt-low)的一个专门的变体。
...
无论我们是否使用了概念的直接语言支持，本条规则都适用。 这是一种一般性的设计规则，即便对于非模板也同样适用：
...
这是最小化的设计，但会使用户遇到意外或受到限制。 可能它还会比较低效。
...
这是一种数学意义上的公理：可以假定成立而无需证明。 通常公理是无法证明的，而即便可以证明，通常也超出了编译器的能力。 一条公理也许并不是通用的，不过模板作者可以假定其对于其所实际使用的所有输入均成立（类似于一种前条件）。

##### 注解

这种上下文中的公理都是布尔表达式。 参见 [Palo Alto TR](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#S-references) 中的例子。 当前，C++ 并不支持公理（即便 ISO Concepts TS 也不支持），我们不得不长期用代码注释来给出它们。 语言支持一旦出现，公理前面的 `//` 就可以删掉了。
...
一个处于开发之中的新“概念”的早期版本，经常会仅仅定义了一组简单的约束而并没有恰当指定的语义。 为其寻找正确的语义可能是很费功夫和时间的。 不过不完整的约束集合仍然是很有用的：
...
这样 `Balancer` 就必须为树的 `Node` 至少提供这些操作， 但我们还是无法指定详细的语义，因为一种新种类的平衡树可能需要更多的操作， 而在设计的早期阶段，很难确定把对于所有节点的精确的一般语义所确定下来。

不完整的或者没有恰当指定的语义的“概念”仍然是很有用的。 比如说，它可能允许在初始的试验之中进行某些检查。 不过，请不要把它当作是稳定的。 它的每个新的用例都可能导致不完整概念的改善。
...
编译器可以基于所要求的操作的集合（这里为前缀 `++`）来确定提炼关系。 这样做减少了这些类型的实现者的负担， 因为他们不再需要任何特殊的声明来“打入概念内部”了。 如果两个概念具有完全相同的要求的话，它们在逻辑上就是等价的（不存在提炼）。

##### 强制实施

- 对与已经出现的另一个概念具有完全相同的要求的概念进行标记（它们中不存在更精炼的概念）。 为对它们进行区分，参见 [T.24](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rt-tag)。
...
特征可以是特征类或者类型特征。 它们可以是用户定义的，或者标准库中的。 优先采用标准库中的特征。
...
仅当 `C<T>` 无法满足时，编译器将会选择无约束的模板。 如果你并不想（或者无法）定义无约束版本的 `f()` 的话，你可以删掉它。
...
有时候会（不正确地）把对单个要求的互补约束当做是可以接受的。 不过，对于两个或更多的要求来说，所需要的定义的数量是按指数增长的（2，4，8，16，……）：
...
显然，直接使用标准的 `equality_comparable` 要更好而且更容易， 但是——只是一个例子——如果你不得不定义这样的概念的话，应当这样：
...
而不是定义两个无意义的概念 `has_equal` 和 `has_not_equal` 仅用于帮助 `Equality` 的定义。 “无意义”的意思是我们无法独立地指定 `has_equal` 的语义。
...
这些年以来，使用模板的编程一直忍受着模板的接口及其实现之间的 微弱区分性。 在引入概念之前，这种区分是没有直接的语言支持的。 不过，模板的接口是一个关键概念——是用户和实现者之间的一种契约——而且应当进行周密的设计。
...
毕竟，`sortable` 里面并没有要求任何 `iostream` 支持。 而另一方面，在排序的基本概念中也没有任何东西是有关于调试的。

##### 注解

如果我们要求把用到的所有操作都在要求部分中列出的话，接口就会变得不稳定： 每当我们改动了调试设施，使用情况数据收集，测试支持，错误报告等等， 模板的定义就都得进行改动，而模板的所有使用方都不得不进行重新编译。 这样做很是累赘，而且在某些环境下是不可行的。

与之相反，如果我们在实现中使用了某个并未被概念检查提供保证的操作的话， 我们可能会遇到一个延后的编译时错误。

通过不对模板参数的那些不被认为是基本的性质进行概念检查， 我们把其检查推迟到了进行实例化的时候。 我们认为这是值得做出的折衷。
...
注意，C++17 强允许模板参数直接从构造函数参数进行推断，而使这条规则变得多余： [构造函数的模板形参推断(Rev. 3)](http://www.open-std.org/jtc1/sc22/wg21/docs/papers/2016/p0091r1.html)。 例如：
...
这里 `Bad` 中的 `==` 有意设计为造成问题，不过你是否在真实代码中发现过这个问题呢？ 问题在于 `v.size()` 返回的是 `unsigned` 整数，因此需要进行转换才能调用局部的 `==`； 而 `Bad` 中的 `=` 则不需要任何转换。 实际的类型，比如标准库的迭代器，也可以被弄成类似的反社会倾向。
...
请当心[互补约束](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rt-not)。 当用 `enable_if` 来模拟概念重载时，有时候会迫使我们使用这种易错的设计技巧。
...
模板的定义式（无论是类还是函数）都可能包含任意的代码，因而只有相当范围的 C++ 编程技术才能覆盖这个主题。 不过，这个部分所关注的是特定于模板的实现的问题。 特别地说，这里关注的是模板定义式对其上下文之间的依赖。
...
让模板仅对其参数进行操作是一种把依赖减到最少的方式，但通常这是很难做到的。 比如说，一个算法通常会使用其他的算法，并且调用一些并非仅在参数上做动作的一些操作。 而且别再让我们从宏开始干活！

**参见**：[T.69](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rt-customization)。
...
这看起来没什么问题，但现在 `Link` 形成依赖于分配器（尽管它不使用分配器）。 这迫使冗余的实例化在某些现实场景中可能造成出奇的高的成本。 通常，解决方案是使用自己的最小模板参数集使嵌套类非局部化。
...
这条规则的更一般化的版本是， “如果类模板的成员依赖于 M 个模板参数中的 N 个，就将它置于只有 N 个参数的基类之中。” 当 N == 1 时，可以如同 [T.61](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rt-scary) 一样在一个基类和其外围作用域中的一个类之间进行选择。
...
特征通常是用以计算一个类型的类型别名， 用以计算一个值的 `constexpr` 函数， 或者针对用户的类型进行特化的传统的特征模板。

##### 注解

当你打算为依赖于某个模板类型参数的值 `t` 调用自己的辅助函数 `helper(t)` 时， 请将函数放入一个 `::detail` 命名空间中，并把调用限定为 `detail::helper(t);`。 无限定的调用将成为一个定制点，它将会调用处于 `t` 的类型所在命名空间中的任何 `helper` 函数； 这可能会导致诸如[意外地调用了无约束函数模板](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rt-visible)这样的问题。
...
在这之中，编译器不知道 `Vector<int>::sort()` 是不是会被调用，因此它必须为之生成代码。 `Vector<string>::sort()` 也与此相似。 除非这两个函数被调用，否则这就是代码爆炸。 不难想象当一个带有几十个成员函数和几十个派生类的类层次被大量实例化时会怎么样。

##### 注解

许多情况下都可以通过不为基类进行参数化而提供一个稳定的接口； 参见[“稳定的基类”](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rt-abi)和 [OO 与 GP](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rt-generic-oo)。
...
`aa[0]` 可能会变为 `Pear`（并且没进行过强制转换！）。 当 `sizeof(Apple) != sizeof(Pear)` 时，对 `aa[1]` 的访问就是并未跟数组中的对象的适当起始位置进行对齐的。 这里出现了类型违例，以及很可能出现的内存损坏。 决不要写这样的代码。

注意，`maul()` 违反了 [`T*` 应指向独立对象的规则](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rf-ptr)。
...
注意，`maul2()` 中的赋值违反了[避免发生切片的规则](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Res-slice)。
...
这样的话就只有一份用于对 `List` 的元素进行入链和解链的操作的代码了。 而类 `Link` 和 `List` 除了进行类型操作之外什么也没做。

除了使用一个独立的“base”类型外，另一种常用的技巧是对 `void` 或 `void*` 进行特化，并让针对 `T` 的通用模板成为在从或向 `void` 的核心实现进行强制转换的一层类型安全封装。

**替代方案**: 使用一个 [PImpl](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Ri-pimpl) 实现。
...
元编程，是其中至少一项输入或者一项输出是类型的编程。 模板提供了编译期的图灵完备（除了内存消耗外）的鸭子类型系统。 其语法和所需技巧都相当可怕。
...
请使用概念来替代它。不过请参见[如何在没有语言支持时模拟概念](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rt-emulate)。
...
**替代方案**: 如果结果是一个值而不是类型，请使用 [`constexpr` 函数](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rt-fct)。
...
参见 [F.10](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rf-name)

### T.141: 当仅在一个地方需要一个简单的函数对象时，使用无名的 lambda

参见 [F.11](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rf-lambda)
...
C 和 C++ 是联系很紧密的两门语言。 它们都是源于 1978 年的“经典 C”语言的，且从此之后就在 ISO 标准委员会中进行演化。 为了让它们保持兼容，我们做过许多努力，但它们各自都并非是对方的子集。

C 规则概览：

- [CPL.1: 优先使用 C++ 而不是 C](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rcpl-c)
- [CPL.2: 当一定要用 C 时，应使用 C 和 C++ 的公共子集，并将 C 代码以 C++ 来编译](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rcpl-subset)
- [CPL.3: 当一定要用 C 来作为接口时，应在使用这些接口的调用方代码中使用 C++](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rcpl-interface)
...
针对在 C 中从 `void*` 或向它进行的隐式强制转换的相关规则比较麻烦而且并未强制实施。 特别是，这个例子违反了禁止把类型转换为具有更严格对齐的类型的规则。
...
例如，为使用第三方 C 程序库或者 C 系统接口，可以使用 C 和 C++ 的公共子集来定义其底层接口，以获得更好的类型检查。 尽可能将底层接口封装到一个遵循了 C++ 指导方针的接口之中（以获得更好的抽象、内存安全性和资源安全性），并在 C++ 代码中使用这个 C++ 接口。
...
区分声明（用作接口）和定义（用作实现）。 用头文件来表达接口并强调逻辑结构。

源文件规则概览：

- [SF.1: 如果你的项目还未采用别的约定的话，应当为代码文件使用后缀 `.cpp`，而对接口文件使用后缀 `.h`](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rs-file-suffix)
- [SF.2: 头文件不能含有对象定义或非内联的函数定义](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rs-inline)
- [SF.3: 对在多个源文件中使用的任何声明，都应使用头文件](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rs-declaration-header)
- [SF.4: 在文件中的其他所有声明之前包含头文件](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rs-include-order)
- [SF.5: `.cpp` 文件必须包含定义了它的接口的一个或多个头文件](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rs-consistency)
- [SF.6: `using namespace` 指令，（仅）可以为迁移而使用，可以为基础程序库使用（比如 `std`），或者在局部作用域中使用](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rs-using)
- [SF.7: 请勿在头文件中的全局作用域使用 `using namespace` 指令](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rs-using-directive)
- [SF.8: 为所有的头文件使用 `#include` 防卫宏](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rs-guards)
- [SF.9: 避免源文件的循环依赖](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rs-cycles)
- [SF.10: 避免依赖于隐含地 `#include` 进来的名字](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rs-implicit)
- [SF.11: 头文件应当是自包含的](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rs-contained)
- [SF.12: 对相对于包含文件的文件优先采用引号形式的 `#include`，其他情况下采用角括号形式](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rs-incform)
- [SF.13: 在 `#include` 语句中使用可移植的标识符](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rs-portable-header-id)
- 
- [SF.20: 用 `namespace` 表示逻辑结构](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rs-namespace)
- [SF.21: 请勿在头文件中使用无名（匿名）命名空间](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rs-unnamed)
- [SF.22: 为所有的内部/不导出的实体使用无名（匿名）命名空间](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rs-unnamed2)

### SF.1: 如果你的项目还未采用别的约定的话，应当为代码文件使用后缀 `.cpp`，而对接口文件使用后缀 `.h`

参见 [NL.27](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rl-file-suffix)
...
`bar` 的维护者在需要改变 `bar` 的类型时，无法找到其全部声明。 `bar` 的使用者不知道他所使用的接口是否完整和正确。顶多会从连接器获得一些（延迟的）错误消息。
...
有一种论点是通过在打算保护的代码的*后面*再 `#include` 头文件，以此将代码同头文件中的声明式和宏等之间进行隔离 （如上面例子中标为“不好”之处）。 不过，
...
`foobar` 的返回类型错误在编译 `foo.cpp` 时立即就被发现了。 对 `bar` 的参数类型错误在连接时之前无法被发现，因为可能会有重载发生，但系统性地使用 `.h` 文件能够增加时其被程序员更早发现的可能性。
...
不过，不大可能导致并非错误的名字解析， 假定使用 `using namespace std` 的人们都了解 `std` 以及这种风险。

##### 注解

`.cpp` 文件也是一种形式的局部作用域。 包含一条 `using namespace X` 的 N 行的 `.cpp` 文件中发生名字冲突的机会， 和包含一条 `using namespace X` 的 N 行的函数， 以及每个都包含一条 `using namespace X` 的总行数为 N 行的 M 个函数，没有多少差别。

##### 注解

[请勿在头文件全局作用域中使用 `using namespace`](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rs-using-directive)。
...
一个例外是 `using namespace std::literals;`。若要在头文件中使用 字符串字面量，则必须如此，而且根据[规则](http://eel.is/c++draft/over.literal)——用户必须以 `operator""_x` 来命名他们自己的 UDL——它们并不会与标准库相冲突。
...
一些实现提供了如 `#pragma once` 这样的厂商扩展作为包含防卫宏的替代。 这并非标准且不可移植。它向程序中注入了宿主机器的文件系统的语义， 而且把你锁定到某个特定厂商。 我们的建议是编写 ISO C++：参见[规则 P.2](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rp-cplusplus)。
...
`<iostream>` 暴露了 `std::string` 的定义（“为什么？”是一个有趣的问题）， 但其并不必然是通过传递包含整个 `<string>` 头文件而做到这一点的， 这带来了常见的新手问题“为什么 `getline(cin,s);` 不成？”， 甚至偶尔出现的“`string` 无法用 `==` 来比较”。
...
强制实施将需要一些有关头文件中哪些是“导出”给用户所用而哪些是用于实现的知识。 在我们能用到模块之前没有真正的好方案。
...
[标准](http://eel.is/c++draft/cpp.include) 向编译器提供了对于实现 使用角括号（`<>`）或引号（`""`）语法的 `#include` 的两种形式的灵活性。 各厂商利用了这点并采用了不同的搜索算法和指定包含路径的方法。
...
如果只使用纯语言本身的话，任何开发任务都会变得很麻烦（无论以何种语言）。 如果使用了某个合适的程序库的话，则任何开发任务都会变得相当简单。

这些年来标准库一直在持续增长。 现在它在标准中的描述已经比语言功能特性的描述更大了。 因此，可能指导方针的库部分的规模最终将会增长等于甚至超过其他的所有部分。

« ??? 我们需要另一个层次的规则编号 ??? »

C++ 标准库组件概览：

- [SL.con: 容器](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#SS-con)
- [SL.str: 字符串](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#SS-string)
- [SL.io: I/O 流（iostream）](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#SS-io)
- [SL.regex: 正则表达式](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#SS-regex)
- [SL.chrono: 时间](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#SS-chrono)
- [SL.C: C 标准库](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#SS-clib)

标准库规则概览：

- [SL.1: 尽可能使用程序库](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rsl-lib)
- [SL.2: 优先使用标准库而不是其他程序库](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rsl-sl)
- [SL.3: 请勿向命名空间 `std` 中添加非标准实体](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#sl-std)
- [SL.4: 以类型安全的方式使用标准库](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#sl-safe)
- ???
...
本条规则是半哲学性的元规则，需要许多具体规则予以支持。 我们需要将之作为对于更加专门的规则的总括。
...
- [SL.con.1: 优先采用 STL 的 `array` 或 `vector` 而不是 C 数组](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rsl-arrays)
- [SL.con.2: 除非有理由使用别的容器，否则默认情况应优先采用 STL 的 `vector`](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rsl-vector)
- [SL.con.3: 避免边界错误](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rsl-bounds)
- [SL.con.4: 请勿对非可平凡复制的实参使用 `memset` 或 `memcpy`](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rsl-copy)
...
在栈上分配的固定大小的数组和把元素都放在自由存储上的 `vector` 之间比较性能是没什么意义的。 你同样也可以在栈上的 `std::array` 和通过指针访问 `malloc()` 的结果之间进行这样的比较。 对于大多数代码来说，即便是栈上分配和自由存储分配之间的差异也没那么重要，但 `vector` 带来的便利和安全性却是重要的。 如果有人编写的代码中这种差异确实重要，那么他显然可以在 `array` 和 `vector` 之间做出选择。
...
使用 `()` 初始化来将 `vector` 初始化为具有特定数量的元素。 使用 `{}` 初始化来以一个元素列表来对 `vector` 进行初始化。
...
[优先采用 `{}` 初始化式语法](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Res-list)。
...
应用于一组元素的范围的标准库函数，都有（或应当有）接受 `span` 的边界安全重载。 如 `vector` 这样的标准类型，在边界剖面配置下（以某种不兼容的方式，如添加契约）可以被修改为实施边界检查，或者使用 `at()`。

理想情况下，边界内保证应当可以被静态强制实行。 例如：
...
通常，可以用一个简单的预先检查来消除检查每个索引的需要。 例如
...
本条规则属于[边界剖面配置](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#SS-bounds)。
...
文本处理是一个大的主题。 `std::string` 无法全部覆盖这些。 这一部分主要尝试澄清 `std::string` 和 `char*`、`zstring`、`string_view` 和 `gsl::span<char>` 之间的关系。 有关非 ASCII 字符集和编码的重要问题（比如 `wchar_t`，Unicode，以及 UTF-8 等）将在别处讨论。

**参见**：[正则表达式](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#SS-regex)

在这里，我们用“字符序列”或“字符串”来代表（终将）作为文本来读取的字符序列。 We don’t consider ???

字符串概览：

- [SL.str.1: 使用 `std::string` 以拥有字符序列](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rstr-string)
- [SL.str.2: 使用 `std::string_view` 或 `gsl::span` 以指代字符序列](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rstr-view)
- [SL.str.3: 使用 `zstring` 或 `czstring` 以指代 C 风格、以零结尾的字符序列](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rstr-zstring)
- [SL.str.4: 使用 `char*` 以指代单个字符](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rstr-char*)
- [SL.str.5: 使用 `std::byte` 以指代并不必须表示字符的字节值](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rstr-byte)
- [SL.str.10: 当需要实施相关于文化地域的操作时，使用 `std::string`](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rstr-locale)
- [SL.str.11: 当需要改动字符串时，使用 `gsl::span` 而不是 `std::string_view`](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rstr-span)
- [SL.str.12: 为作为标准库的 `string` 类型的字符串字面量使用后缀 `s`](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rstr-s)

**参见**：

- [F.24 span](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rf-range)
- [F.25 zstring](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rf-zstring)
...
我们搞对了吗？ 调用者能记得要对返回的指针调用 `free()` 吗？ 这段代码能通过安全性评审吗？

##### 注解

没有测量就不要假设 `string` 比底层技术慢，要记得并非所有代码都是性能攸关的。 [请勿进行不成熟的优化](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rper-knuth)
...
`std::string_view` 或 `gsl::span<char>` 提供了简易且（潜在）安全的对字符序列的访问，并与序列的 分配和存储方式无关。
...
参见 [`zstring`](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rstr-zstring)，[`string`](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rstr-string)，以及 [`string_view`](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rstr-view)。
...
`std::string` 支持标准库的 [`locale` 功能](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rstr-locale)
...
`iostream` 是一种类型安全的，可扩展的，带格式的和无格式的流式 I/O 的 I/O 程序库。 它支持多种（且用户可扩展的）缓冲策略以及多种文化地域。 它可以用于进行便利的 I/O，内存读写（字符串流）， 以及用户定义的扩展，诸如跨网络的流（asio：尚未标准化）。

I/O 流规则概览：

- [SL.io.1: 仅在必要时才使用字符层面的输入](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rio-low)
- [SL.io.2: 当进行读取时，总要考虑非法输入](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rio-validate)
- [SL.io.3: 优先使用 iostream 进行 I/O](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rio-streams)
- [SL.io.10: 除非你使用了 `printf` 族函数，否则要调用 `ios_base::sync_with_stdio(false)`](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rio-sync)
- [SL.io.50: 避免使用 `endl`](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rio-endl)
...
对于 `cin`/`cout`（或同等设备）的交互来说，没什么原因必须进行冲洗；它们是自动进行的。 对于向文件写入来说，也很少需要 `flush`。

##### 注解

对于字符串流（指 `ostringstream`），插入一个 `endl` 完全等价于 插入一个 `'\n'` 字符，但正是这种情况下，`endl` 可能会明显比较慢。
...
`<chrono>`（在命名空间 `std::chrono` 中定义）提供了 `time_point` 和 `duration`，并同时提供了 用于以各种不同单位输出时间的函数。 它还提供了用于注册 `time_point` 的时钟。
...
- [SL.C.1: 请勿使用 setjmp/longjmp](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rclib-jmp)
- [???](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#???)
- [???](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#???)
...
- [A.1: 分离稳定的代码和不稳定的代码](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Ra-stable)
- [A.2: 将潜在可复用的部分作为程序库](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Ra-lib)
- [A.4: 程序库之间不能有循环依赖](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Ra-dag)
- [???](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#???)
- [???](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#???)
- [???](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#???)
- [???](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#???)
...
本部分包含一些在不少地方流行的规则和指导方针，但是我们慎重地建议不要采纳它们。 我们完全了解这些规则曾经在某些时间和场合是有意义的，而且我们自己也曾经采用过它们。 不过，在我们所推荐并以各项指导方针所支持的编程风格的情况中，这些“伪规则”是有害的。

即便是今天，仍有一些情况下这些规则是有意义的。 比如说，缺少合适的工具支持会导致异常在硬实时系统中的不适用， 但请不要盲目地信任“通俗智慧”（比如有关“效率”的未经数据支持的观点）； 这种“智慧”也许是基于几十年前的信息，或者是来自于与 C++ 有非常不同性质的语言的经验 （比如 C 或者 Java）。

对于这些伪规则的替代方案的正面观点都在各个规则的“替代方案”部分中给出。

伪规则概览：

- [NR.1: 请勿坚持认为声明都应当放在函数的最上面](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rnr-top)
- [NR.2: 请勿坚持使函数中只保留一个 `return` 语句](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rnr-single-return)
- [NR.3: 请勿避免使用异常](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rnr-no-exceptions)
- [NR.4: 请勿坚持把每个类定义放在其自己的源文件中](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rnr-lots-of-files)
- [NR.5: 请勿采用两阶段初始化](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rnr-two-phase-init)
- [NR.6: 请勿把所有清理操作放在函数末尾并使用 `goto exit`](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rnr-goto-exit)
- [NR.7: 请勿使所有数据成员 `protected`](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rnr-protected-data)
- ???
...
- [坚持为对象进行初始化](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Res-always)。
- [ES.21: 不要在确实需要使用变量（或常量）之前就引入它](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Res-introduce)。
...
注意我们（故意地）违反了禁止未初始化变量的规则，因为这种风格通常都会导致这样。 而且，这种风格也会倾向于采用 [goto exit](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rnr-goto-exit) 伪规则。
...
- [RAII](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Re-raii)
...
- 使用异常和 [RAII](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Re-raii)
- 对于非 RAII 资源，使用 [`finally`](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Re-finally)。
...
- [使成员数据 `public` 或者（更好地）`private`](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rh-protected)。

# RF: 参考材料

已经为 C++，尤其是对 C++ 的使用编写过了许多的编码标准、规则和指导方针。 它们中许多都
...
不良的编码标准要比没有编码标准还要差。 不过一组恰当的指导方针比没有标准要好得多：“形式即解放。”

我们为什么不能有一种允许所有我们想要的同时又禁止所有我们不期望的东西的语言（“完美的语言”）呢？ 本质上说，这是由于可负担的语言（及其工具链）同时也要为那些需求与你不同的人提供服务，并且要为你今后比今天更多的需求提供服务。 而且，你的需求会随时间而改变，而为此你则需要采用一种通用语言。 今日貌似理想的语言在未来可能会变得过于受限了。

编码指导方针可以使语言能够适应于特定的需求。 因此，并不存在适用于每个人的单一编码风格。 我们预计不同的组织会提供更具限制性和更严格的编码风格附加规定。

参考材料部分：

- [RF.rules: 编码规则](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#SS-rules)
- [RF.books: 带有编码指导方针的书籍](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#SS-books)
- [RF.C++: C++ 编程 (C++11/C++14/C++17)](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#SS-cplusplus)
- [RF.web: 网站](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#SS-web)
- [RS.video: 有关“当代 C++”的视频](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#SS-vid)
- [RF.man: 手册](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#SS-man)
- [RF.core: 核心指导方针相关材料](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#SS-core)
...
- Facebook: ???
- [GCC Coding Conventions](https://gcc.gnu.org/codingconventions.html). C++03 以及（相当）一部分向后兼容。
- [Google C++ Style Guide](https://google.github.io/styleguide/cppguide.html). 面向 C++17 和（同样）较老的代码库。Google 的专家们现在正展开活跃的合作，以改进这里的各项指导方针，有希望能够合并这些成果，以使它们能够成为他们也同样推荐采纳的一组现代的通用指导方针。
- [JSF++: JOINT STRIKE FIGHTER AIR VEHICLE C++ CODING STANDARDS](http://www.stroustrup.com/JSF-AV-rules.pdf). 文档编号 2RDU00001 Rev C. December 2005. 针对飞行控制软件。 针对硬实时。 这意味着它需要非常多的限制（“程序如果发生故障就会有人挂掉”）。 例如，飞机起飞后禁止进行任何自由存储的分配和回收（禁止内存溢出并禁止发生碎片化）。 禁止使用异常（因为没有可用工具可以保证异常能够在固定的短时间段内被处理）。 所使用的程序库必须是已被证明可以用于关键任务应用的。 它和这个指导方针集合的相似性并不让人惊讶，因为 Bjarne Stroustrup 正是 JSF++ 的作者之一。 建议采纳，但请注意其非常特定的关注领域。
- [MISRA C++:2023 Guidelines for the use C++17 in critical systems](https://misra.org.uk/product/misra-cpp2023/)。
- [Using C++ in Mozilla Code](https://firefox-source-docs.mozilla.org/code-quality/coding-style/using_cxx_in_firefox_code.html). 如其名称所示，它关注于跨许多（老）编译器的兼容性。 因此，它是很具有限制性的。
...
- [SEI CERT: Secure C++ Coding Standard](https://wiki.sei.cmu.edu/confluence/x/Wnw-BQ). 针对安全关键代码所编写的一组非常好的规则（还带有示例和原理说明）。 它们的许多规则都广泛适用。
- [High Integrity C++ Coding Standard](http://www.codingstandard.com/).
- [llvm](http://llvm.org/docs/CodingStandards.html). 有些简略，基于 C++14，而且是（有理由地）针对其应用领域的。
- ???

## RF.books: 带有编码指导方针的书籍

- [Meyers96](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Meyers96) Scott Meyers: *More Effective C++*. Addison-Wesley 1996.
- [Meyers97](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Meyers97) Scott Meyers: *Effective C++, Second Edition*. Addison-Wesley 1997.
- [Meyers01](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Meyers01) Scott Meyers: *Effective STL*. Addison-Wesley 2001.
- [Meyers05](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Meyers05) Scott Meyers: *Effective C++, Third Edition*. Addison-Wesley 2005.
- [Meyers15](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Meyers15) Scott Meyers: *Effective Modern C++*. O’Reilly 2015.
- [SuttAlex05](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#SuttAlex05) Sutter and Alexandrescu: *C++ Coding Standards*. Addison-Wesley 2005. 与其说是一组规则，不如说是一组元规则。前 C++11 时代。
- [Stroustrup05](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Stroustrup05) Bjarne Stroustrup: [A rationale for semantically enhanced library languages](http://www.stroustrup.com/SELLrationale.pdf). LCSD05. October 2005.
- [Stroustrup14](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Stroustrup05) Stroustrup: [A Tour of C++](http://www.stroustrup.com/Tour.html). Addison Wesley 2014. 每章的结尾都有一个包含一组建议的忠告部分。
- [Stroustrup13](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Stroustrup13) Stroustrup: [The C++ Programming Language (4th Edition)](http://www.stroustrup.com/4th.html). Addison Wesley 2013. 每章的结尾都有一个包含一组建议的忠告部分。
- Stroustrup: [Style Guide](http://www.stroustrup.com/Programming/PPP-style.pdf) for [Programming: Principles and Practice using C++](http://www.stroustrup.com/programming.html). 大多是一些低级的命名和代码布局规则。 主要作为教学工具。

## RF.C++: C++ 编程 (C++11/C++14)

- [TC++PL4](http://www.stroustrup.com/4th.html): 面向有经验的程序员的，对 C++ 语言和标准库的全面彻底的描述。
- [Tour++](http://www.stroustrup.com/Tour.html): 面向有经验的程序员的，对 C++ 语言和标准库的简介。
- [Programming: Principles and Practice using C++](http://www.stroustrup.com/programming.html): 面向初学者和新手们的教材。
...
- Bjarne Stroustrup: [No Littering!](https://www.youtube.com/watch?v=01zI9kV4h8c). Bay Area ACCU 2016. It gives some idea of the ambition level for the Core uidelines.
...
理想情况，我们应当遵循所有这些指导方针。 这样能够得到最简洁，最规范，最少易错性，而且通常是最快的代码。 不幸的是这通常是不可能的，因为我们不得不让代码适合于大型的代码库并使用一些现存的程序库。 常常是，这样的代码已经编写好几十年了，并且并不遵循这些指导方针。 我们必须以[渐次采纳](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#S-modernizing)为目标。

无论采用何种渐次采纳的策略，我们都应当能够首先采用一些相关指导方针的集合来 处理某些问题的集合，遗留其他的以后处理。 当发现某些而不是全部的指导方针对于代码库有关时，以及当在某个专门化的应用领域 采用一组专门化的指导方针的集合时，会出现类似的“相关指导方针”的主意。 我们称这样的相关指导方针的集合为一个“剖面配置”。 我们对这种指导方针集合的目标是其内聚性，它们一起可以有助于我们达成某个特定的目标，如“消除范围错误” 或“静态类型安全性”。 每个剖面配置都被设计用于消除一个类别的错误。 而“随意”实施一些独立的规则，相对于提供确定的改善来说，更像是对代码库的破坏。

“剖面配置”是确定的并且可移植实施的规则（限制）子集，它们是专门设计以达成某项特定保证的。 “确定性”意味着它们仅需要进行局部分析，并且可以在一台计算机中进行实现（虽然并不比如此）。 “可移植实施性”表明它们和语言规则相似，因而程序员们可以期望不同实施工具对于相同的代码给出相同的答案。

编写成在这样的语言剖面配置下仍免于警告的代码，可以认为是遵循这个剖面配置的。 而遵从的代码则可以认为对于该剖面配置的目标安全属性来说是安全的。 遵从的代码不会成为这种性质的错误的根本原因， 虽然程序中可能从其他的代码引入这样的错误。 剖面配置还可能会引入一些额外的库类型，以简化遵从性并鼓励编写正确的代码。

剖面配置概览：

- [Pro.type: 类型安全性](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#SS-type)
- [Pro.bounds: 边界安全性](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#SS-bounds)
- [Pro.lifetime: 生存期安全性](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#SS-lifetime)

未来，我们打算定义更多的剖面配置，并向现有剖面配置中添加更多的检查。 候选者有：
...
这样 `raw_find()` 就可以在内存中到处爬了。 显然，进行抑制应当是非常罕见的。

## Pro.safety: 类型安全性剖面配置

这个剖面配置将能够简化正确使用类型的代码编写，并避免因疏忽产生类型双关。 它是关注于移除各种主要的类型违例的因素（包括对强制转换和联合的不安全使用）而达成这点的。

针对本部分的目的而言， 类型安全性被定义为这样的性质：对变量的使用不会不遵守其所定义的类型的规则。 通过类型 `T` 所访问的内存，不应该是某个实际上包含了无关类型 `U` 的对象的有效内存。 注意，当和[边界安全性](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#SS-bounds)、[生存期安全性](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#SS-lifetime)组合起来时，安全性才是完整的。

这个剖面配置的实现应当在源代码中识别出下列模式，将之作为不符合并给出诊断信息。

类型安全性剖面配置概览：

- Type.1: [避免强制转换](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Res-casts)：
  1. 请勿使用 `reinterpret_cast`；此为[避免强制转换](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Res-casts)和[优先使用具名的强制转换](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Res-casts-named)的严格的版本。
  2. 请勿在算术类型上使用 `static_cast`；此为[避免强制转换](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Res-casts)和[优先使用具名的强制转换](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Res-casts-named)的严格的版本。
  3. 当源指针类型和目标类型相同时，请勿进行指针强制转换；此为[避免强制转换](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Res-casts)的严格的版本。
  4. 当指针转换可以隐式转换时，请勿使用指针强制转换；此为[避免强制转换](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Res-casts)的严格的版本。
- Type.2: 请勿使用 `static_cast` 进行向下强制转换： [代之以使用 `dynamic_cast`](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rh-dynamic_cast)。
- Type.3: 请勿使用 `const_cast` 强制掉 `const`（亦即不要这样做）： [不要强制掉 `const`](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Res-casts-const)。
- Type.4: 请勿使用 C 风格的强制转换 `(T)expression` 和函数式风格强制转换 `T(expression)`： 优先使用[构造语法](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Res-construct)，[具名的强制转换](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Res-casts-named)，或 `T{expression}`。
- Type.5: 请勿在初始化之前使用变量： [坚持进行初始化](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Res-always)。
- Type.6: 坚持初始化数据成员： [坚持进行初始化](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Res-always)， 可以采用[默认构造函数](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rc-default0)或者 [默认成员初始化式](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rc-in-class-initializer)。
- Type.7: 避免裸 union： [代之以使用 `variant`](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Ru-naked)。
- Type.8: 避免 varargs： [不要使用 `va_arg` 参数](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#F-varargs)。

##### 影响

在类型安全性剖面配置下，你可以相信每个操作都将在有效的对象上进行。 可能抛出异常以报告无法（在编译时）被静态地检测到的错误。 要注意的是，这种类型安全性仅当我们同样具有[边界安全性](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#SS-bounds)和[生存期安全性](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#SS-lifetime)时才是完整的。 而没有这些保证的话，一个内存区域可能以与其所存储的单个或多个对象，或对象的一部分无关的方式被访问。
...
我们将边界安全性定义为这样一种性质：程序不通过一个对象来对分配给这个对象的内存范围之外的内存进行访问。 仅当边界安全性与[类型安全性](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#SS-type)和[生存期安全性](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#SS-lifetime)组合起来时才是完整的， 它们还会包含其他允许发生边界违例的不安全操作。

边界安全性剖面配置概览：

- Bounds.1: 请勿使用指针算术。请使用 `span` 代替： [（仅）传递单个对象的指针](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Ri-array)，并[保持指针算术的简单性](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Res-ptr)。
- Bounds.2: 仅使用常量表达式对数组进行索引操作： [（仅）传递单个对象的指针](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Ri-array)，并[保持指针算术的简单性](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Res-ptr)。
- Bounds.3: 避免数组向指针的退化： [（仅）传递单个对象的指针](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Ri-array)，并[保持指针算术的简单性](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Res-ptr)。
- Bounds.4: 请勿使用不进行边界检查的标准库函数和类型： [以类型安全的方式使用标准库](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rsl-bounds)

##### 影响

边界安全性意味着，当访问对象（尤其是数组）时不会越过对象的内存分配范围。 这消除了一大类的隐伏且难于发现的错误，包括（不）著名的“缓冲区溢出”错误。 这避免了安全漏洞，以及（当越界写入时发生）内存损坏错误的大量来源。 即使越界访问“只是读取操作”，它也可能导致不变式的违反（当所访问的不是预期的类型时） 和“神秘的值”。

## Pro.lifetime: 生存期安全性剖面配置

通过已经不指向任何东西的指针进行访问，是错误的一种主要来源， 而且在许多传统的 C 或 C++ 风格的编程中这很难避免。 例如，指针可能未初始化，值为 `nullptr`，指向越过指针范围，或者指向已删除的对象。

[参见此处的设计说明书的当前版本](https://github.com/isocpp/CppCoreGuidelines/blob/master/docs/Lifetime.pdf)

生存期安全性剖面配置概览：

- Lifetime.1: 不要解引用无效指针： [检测或避免](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Res-deref)。
...
GSL 是一个小型的程序库，其中的设施被设计用于支持本指导方针。 不使用这些设施的话，这些指导方针不得不变得对语言细节过于限制。

核心指导方针支持库是定义在 `gsl` 命名空间中的，其中的名字可能是对标准库和其他著名程序库的名字的别名。通过 `gsl` 命名空间进行的（编译期）间接，使我们可以进行试验，以及对这些支持设施提供局部变体。

GSL 只有头文件，可以在 [GSL: 指导方针支持库](https://github.com/Microsoft/GSL)找到。 支持库中的设施被设计为极为轻量化（零开销），它们相比于传统方案并不会带来任何开销。 当需要时，它们还可以用其他功能“工具化”（比如一些检查）来帮助进行诸如调试等任务。

各指导方针中，除了使用 GSL 中的类型之外，还使用了标准程序库（如 C++17）中的类型。 例如，我们假定有一个 `variant` 类型，但它当前尚未在 GSL 中。 总之，请使用[通过表决进入 C++17 的版本](http://www.open-std.org/jtc1/sc22/wg21/docs/papers/2016/p0088r3.html)。
...
- [GSL.view: 视图](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#SS-views)
- [GSL.owner: 所有权指针](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#SS-ownership)
- [GSL.assert: 断言](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#SS-assertions)
- [GSL.util: 工具](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#SS-utilities)
- [GSL.concept: 概念](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#SS-gsl-concepts)
...
引用都不是所有者（参见 [R.4](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rr-ref)）。注意：有许多机会能让引用存活超过其所指代的对象，如按引用返回局部变量，持有 vector 的某个元素的引用然后进行 `push_back`，绑定到 `std::max(x, y + 1)`，等等。生存期安全性剖面配置的目标就是处理这些事情，但即便如此 `owner<T&>` 也没有意义且不建议使用。
...
`owner` 用于对代码中有所有权的指针进行标记，它们无法更改为使用适当的资源包装。 其原因可能包括：
...
- `not_null<T>` // `T` 通常是某个指针类型（例如 `not_null<int*>` 和 `not_null<owner<Foo*>>`），且不能为 `nullptr`。 `T` 可以是 `==nullptr` 有意义的任何类型。
...
“指针算术”最好在 `span` 之内进行。 指向多个 `char` 但并非 C 风格字符串的 `char*`（比如指向某个输入缓冲区的指针）应当表示为一个 `span`。

- `zstring` // `char*`，假定为 C 风格字符串；亦即以零结尾的 `char` 的序列或者是 `nullptr`
- `czstring` // `const char*`，假定为 C 风格字符串；亦即以零结尾的 `const` `char` 的序列或者是 `nullptr`

逻辑上来说，最后两种别名是没有必要的，但我们并不总是依照逻辑的，它们可以在指向单个 `char` 的指针和指向 C 风格字符串的指针之间明确地进行区分。 并未假定为零结尾的字符序列应当是 `span<char>`，或当因 ABI 问题而不可能时是 `char*`，而不是 `zstring`。
...
- `dyn_array<T>` // 一种容器，动态分配的无增长数组。元素的数量在构造时确定并固定下来。 其元素可改变，除非 `T` 为 `const` 类型。基本上这是一个进行分配并拥有其元素的 `span`。

## GSL.assert: 断言

- `Expects` // 前条件断言。当前放置于函数体内。今后应当移动到声明中。 // `Expects(p)` 当不满足 `p == true` 时会终止程序 // `Expects` 处于一组选项的控制之下（强制，错误消息，对终止程序的替代）
- `Ensures` // 后条件断言。当前放置于函数体内。今后应当移动到声明中。

现在这些断言还是宏（天呐！）而且必须（只）被用在函数定义式之内。 等待标准委员会对于契约和断言语法的确定。 参见使用属性语法的[契约提案](http://www.open-std.org/jtc1/sc22/wg21/docs/papers/2016/p0380r1.pdf)， 比如说，`Expects(p)` 将变为 `[[expects: p]]`。
...
这些概念（类型谓词）借用于 Andrew Sutton 的 Origin 程序库， Range 提案， 以及 ISO WG21 的 Palo Alto TR。 其中许多都与已在 C++20 中成为 ISO C++ 标准的概念十分相似。
...
维持一致的命名和代码布局是很有用的。 即便不为其他原因，也可以减少“我的代码风格比你的好”这类的纷争。 然而，人们使用许多许多的不同代码风格，并狂热地坚持它们（的优缺点）。 而且，大多数的现实项目都包含来自于许多来源的代码，因而通常不可能把所有的代码都标准化为某个单一的代码风格。 经过许多的用户请求给予指导后，我们给出一组规则，当你没有更好的选择时可以使用它们，但真正的目标在于一致性，而不是任何一组特定的规则。 IDE 和工具可以提供辅助（当然也可能造成妨碍）。

命名和代码布局规则：

- [NL.1: 不要在代码注释中说明可以由代码来清晰表达的东西](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rl-comments)
- [NL.2: 在代码注释中说明意图](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rl-comments-intent)
- [NL.3: 保持代码注释简明干脆](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rl-comments-crisp)
- [NL.4: 保持一种统一的缩进风格](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rl-indent)
- [NL.5: 避免在名字中编码类型信息](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rl-name-type)
- [NL.7: 使名字的长度大约正比于其作用域的长度](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rl-name-length)
- [NL.8: 使用一种统一的命名风格](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rl-name)
- [NL.9: 将 `ALL_CAPS`（全大写）仅用于宏的名字](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rl-all-caps)
- [NL.10: 优先采用 `underscore_style`（下划线风格）的名字](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rl-camel)
- [NL.11: 使字面量可阅读](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rl-literals)
- [NL.15: 节制地使用空格](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rl-space)
- [NL.16: 使用一种常规的类成员声明次序](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rl-order)
- [NL.17: 使用从 K&R 衍生出的代码布局](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rl-knr)
- [NL.18: 使用 C++ 风格的声明符布局](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rl-ptr)
- [NL.19: 避免使用容易误读的名字](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rl-misread)
- [NL.20: 不要把两个语句放在同一行中](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rl-stmt)
- [NL.21: 每个声明式（仅）声明一个名字](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rl-dcl)
- [NL.25: 请勿将 `void` 用作参数类型](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rl-void)
- [NL.26: 采用符合惯例的 `const` 写法](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rl-const)
- [NL.27: 为代码文件使用后缀 `.cpp`，而对接口文件使用后缀 `.h`](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rl-file-suffix)

这些问题的大部分都是审美问题，程序员都有很强的个人倾向。 IDE 也都会提供某些默认方案和一组替代方案。 这些规则是作为缺省建议的，如果没有别的理由，请采用它们。

我们收到一些意见称命名和代码布局非常个人化和任意性，我们不应该试图为之“立法”。 我们并不是在“立法”（参见前一个段落）。 不过，我们也收到了大量的针对某些命名和代码布局约定的请求，要求当没有外来限制的时候应当采用它们。

更专门和详细的规则更加易于强制实施。

这些规则恐怕会和 [PPP Style Guide](http://www.stroustrup.com/Programming/PPP-style.pdf) 中的建议有很强的相似性， 它是为支持 Stroustrup 的 [Programming: Principles and Practice using C++](http://www.stroustrup.com/programming.html) 而编制的。
...
像 C++ 这样，一些代码风格对类型和非类型之间进行区分。 例如，对类型名字首字母大写，而函数和变量名字则不这样做。
...
避免包含双下划线 `__` 或以一个下划线跟着一个大写字母开头（例如 `_Throws`）的标识符命名。 这种标识符为 C++ 实现所保留。

##### 示例

[Stroustrup](http://www.stroustrup.com/Programming/PPP-style.pdf)： 采用 ISO 标准，但在自己的类型和概念上采用大写字母：
...
尤其是，这避免混淆宏和非宏符号常量（另见 [Enum.5: 请勿为枚举符采用 `ALL_CAPS` 命名方式](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Renum-caps)）
...
这条规则仅作为当你有选择权时的缺省方案。 通常你是没有什么选择权的，而只能遵循某个已经设立的风格以维持[一致性](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rl-name)。 对一致性的需要优先于个人喜好。

这个推荐适用于[当你没有约束条件或者没有更好的想法时](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#S-naming)的情况。 经过很多要求给予指导后，添加这个规则。
...
不能在代码中到处当做[“魔法常量”](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Res-magic)一样乱用字面量， 但当定义它们时使它们更可读仍是个好主意。 在较长的整数串中很容易出现拼写错误。
...
有时候，成员的默认顺序，与将公开接口从实现细节中分离出来的需求之间有冲突。 这种情况下，私有类型和函数可以和私有数据放在一起。
...
用宏来声明成员组的做法通常会导致违反所有的次序规则。 不过，宏的使用掩盖了其所表达的东西。
...
相关细节，参见 [ES.10](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Res-name-one)、
...
即便是 Dennis Ritchie 自己都认为 `void f(void)` 很讨厌。 你可以反驳称，在 C 中当函数原型很少见时禁止这样的代码：
...
我们知道你可能会说“不好”的例子比标有“OK”的更符合逻辑， 但它们会让更多人搞混，尤其是那些依赖于采用了远为常用，符合惯例的 OK 风格的教学材料的新手们。
...
这项约定反应了一种常见使用模式： 头文件更容易和 C 语言共享使用，并可以作为 C++ 和 C 编译，它们通常用 `.h` 后缀， 并且对于有意要和 C 共用的头文件来说，让所有头文件都使用 `.h` 而不是别的扩展名要更加容易。 另一方面，实现文件则很少会和 C 共用，通常应当和 `.c` 文件相区别， 因此一般最好为所有的 C++ 实现文件用别的扩展名（如 `.cpp`）来命名。

特定的名字 `.h` 和 `.cpp` 并不是必要的（只是作为缺省建议），其他的名字也被广泛采用。 例子包括 `.hh`，`.C`，和 `.cxx` 等。请以类似方式使用这些名字。 本文档中我们把 `.h` 和 `.cpp` 作为头文件和实现文件的简便提法， 虽然实际上的扩展名可能是不同的。
...
请参见[本页面开头](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#S-abstract)。这是一个开源项目，旨在为采用当今的 C++ 标准来编写 C++ 代码而维护的一组现代的权威指导方针。这些指导方针的设计是现代的，尽可能使机器可实施的，并且是为贡献和分支保持开放，以使各种组织机构可以便于将它们整合到其自己组织的编码指导方针之中。
...
不一样。`dyn_array` 是一种容器，这与 `vector` 类似，但不可改变它的大小；其大小在运行时它被构造时确定。 它是一种指代动态“堆”分配的固定大小数组的一种安全方式。与 `vector` 不同，它是为了取代数组 `new[]` 的。与委员会中提案的 `dynarray` 不同，它并不会参与编译器和语言的魔法，来在当它作为分配于栈上的对象的成员时也在栈上分配；它只不过指代一个“动态的”或基于堆的数组而已。

### FAQ.59: `Expects` 和 `assert` 一样吗？
...
理想情况下，我们的所有代码都应当遵循全部的规则。 而实际情况则是，我们不得不对付大量的老代码：
...
如果有上百万行的新代码的话，“立刻改掉它们”的想法一般都是不现实的。 因此，我们需要一种方式能够渐进地对代码库进行现代化转换。

将老代码升级为现代风格可能是很让人却步的工作。 老代码通常即混乱（难于理解）又可以（在当前使用范围内）正确工作。 很有可能，原来的程序员并不在场，而且测试用例也不完整。 代码的混乱性显著地增大了为进行任何改动所需要的工作量和引入错误的风险。 通常，混乱的老代码的运行会有不必要的缓慢，因为它需要过期的编译器，并且无法得益于当代硬件的改进。 许多情况下，都需要某种自动进行“现代化转换”的工具支持来进行主要的升级工作。

对代码现代化转换的目的在于简化新功能的添加，简化维护工作，以及增加性能（吞吐量或延迟），和更好地利用当代的硬件能力。 而让代码“看起来更好”或“遵循现代风格”自身并不能成为改动的理由。 每一种改动都蕴含着风险，而是由过时的代码库则会蕴含一些成本（包含丢失机会带来的成本）。 成本的缩减必须超过风险。

如何做呢？

并不存在唯一的代码现代化转换的方案。 如何最好地执行，依赖于代码，更新的进度压力，开发者的背景，以及可用的工具。 下面是一些（非常一般的）想法：

- 理想情况是“对全部代码一起进行升级”。这将在最短的总时间内获得做大的好处。 在大多数情况下，这也是不可能的。
- 我们可以对代码库以模块为单位进行转换，不过任何影响接口（尤其是 ABI）的规则，如 [使用 `span`](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#SS-views)，都无法按模块来达成。
...
这个部分包含了对规则和规则集合的跟进材料。 尤其是，我们列出了更多的原理说明，更长的例子，以及对替代方案的探讨等。

### 讨论: 以成员的声明顺序进行数据成员的定义和初始化
...
[[Cline99\]](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Cline99) §22.03-11, [[Dewhurst03\]](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Dewhurst03) §52-53, [[Koenig97\]](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Koenig97) §4, [[Lakos96\]](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Lakos96) §10.3.5, [[Meyers97\]](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Meyers97) §13, [[Murray93\]](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Murray93) §2.1.3, [[Sutter00\]](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Sutter00) §47
...
- *使用虚基类语义：* 语言规则要求由最终派生类的构造函数来决定调用哪个基类构造函数；你可以利用这点。（参见[[Taligent94\]](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Taligent94)。）
...
- 分配被限定为使用 `operator new`。不过，`B` 可以覆盖 `new`（参见 [SuttAlex05](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Suttalex05) 条款 45 和 46）。
...
**参考**: [[Alexandrescu01\]](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Alexandrescu01) §3, [[Boost\]](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Boost), [[Dewhurst03\]](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Dewhurst03) §75, [[Meyers97\]](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Meyers97) §46, [[Stroustrup00\]](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Stroustrup00) §15.4.3, [[Taligent94\]](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Taligent94)
...
**参考**: [[SuttAlex05\]](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#SuttAlex05) Item 50, [[Cargill92\]](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Cargill92) pp. 77-79, 207? [[Cline99\]](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Cline99) §21.06, 21.12-13? [[Henricson97\]](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Henricson97) pp. 110-114? [[Koenig97\]](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Koenig97) Chapters 4, 11? [[Meyers97\]](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Meyers97) §14? [[Stroustrup00\]](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Stroustrup00) §12.4.2? [[Sutter02\]](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Sutter02) §27? [[Sutter04\]](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Sutter04) §18
...
> 当在栈回溯过程中所调用的析构函数因为异常而退出时，将调用 terminate (15.5.1)。因此析构函数通常应当捕获异常，并防止它们被传播出析构函数。 –[[C++03\]](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Cplusplus03) §15.2(3)
>
> C++ 标准库中所定义的任何析构函数（也包括用于实例化标准库模板的任何类型的析构函数）的操作都不会抛出异常。 –[[C++03\]](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Cplusplus03) §17.4.4.8(3)

包括专门重载的 `operator delete` 和 `operator delete[]` 在内的回收函数也属于这一类别，因为一般它们也被用在清理过程，尤其是在异常处理过程中，用以对部分完成的工作进行撤回。 除了析构函数和回收函数之外，一般的错误安全性技术也依赖于永不失败的 `swap` 操作——这种情况下，它们不仅用于实现确保成功的回滚操作，也用于实现确保成功的提交操作。例如，以下是对类型 `T` 的一种惯用的 `operator=` 实现，它在复制构造之后，调用了无失败的 `swap`：
...
**参考**: [[SuttAlex05\]](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#SuttAlex05) Item 51; [[C++03\]](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Cplusplus03) §15.2(3), §17.4.4.8(3)? [[Meyers96\]](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Meyers96) §11? [[Stroustrup00\]](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Stroustrup00) §14.4.7, §E.2-4? [[Sutter00\]](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Sutter00) §8, §16? [[Sutter02\]](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Sutter02) §18-19
...
**例外**: 当特殊函数的声明仅为了使其非公开或者为虚函数而没有特殊的语义时，它并不导致需要其他的特殊成员。 少数情况下，带有奇怪类型的成员（诸如引用成员）的类也是例外，因为它们的复制语义很古怪。 在持有引用的类中，你可能需要编写复制构造函数和赋值运算符，但预置的析构函数仍能够做出正确的处理。（需要注意，基本上使用引用成员几乎总是错误的。）

**参考**: [[SuttAlex05\]](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#SuttAlex05) Item 52; [[Cline99\]](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Cline99) §30.01-14? [[Koenig97\]](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Koenig97) §4? [[Stroustrup00\]](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Stroustrup00) §5.5, §10.4? [[SuttHysl04b\]](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#SuttHysl04b)

资源管理规则概览：

- [提供强资源安全性；亦即，绝不让你认为是资源的任何东西发生泄漏](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Cr-safety)
- [绝不在持有未被句柄所拥有的资源时返回或抛出异常](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Cr-never)
- [“原生”的指针或引用不可能是资源句柄](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Cr-raw)
- [绝不让指针的生存期超过其所指向的对象](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Cr-outlive)
- [用模板来表现容器（和其他资源句柄）](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Cr-templates)
- [按值返回容器（依靠移动或复制消除来获得性能）](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Cr-value-return)
- [若类为资源句柄，则它需要构造函数，析构函数，复制以及移动操作](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Cr-handle)
- [若类为容器，则应为其提供一个初始化式列表构造函数](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Cr-list)
...
这个类是一个资源句柄。它管理各个 `T` 对象的生存期。为此，`Vector` 必然要对[复制、移动和析构操作](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rc-five)进行定义或弃置。
...
检查器必须将任何“裸指针”当作可疑处理。 检查器可能必须依赖于人工提供的资源列表进行工作。 上手时，我们知道标准库容器，`string`，以及智能指针。 `span` 和 `string_view` 的使用能够提供巨大的帮助（它们并非资源句柄）。
...
**参见**：[F.20，有关“输出（Out）”值的一般条款](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rf-out)
...
见 [F.20](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#Rf-out) 中的例外。
...
一般来说，工具是无法知道类是否是资源句柄的。不过，如果类带有某种[默认操作](https://lynnboy.github.io/CppCoreGuidelines-zh-CN/CppCoreGuidelines-zh-CN#SS-ctor)的话, 它就得拥有全部，而如果类中有成员为资源句柄的话，它也应被当做是资源句柄。
...
这个部分列出了直接支持采用 C++ 核心指导方针的一些工具。这个列表并非要穷尽那些有助于编写良好的 C++ 代码的工具。 如果一个工具被专门设计以支持并关联到 C++ 核心指导方针，那它就是包括进来的候选者。

### 工具: [Clang-tidy](http://clang.llvm.org/extra/clang-tidy/checks/list.html)
...
这是在指导方针中用到的一些术语的相对非正式的定义 （基于 [Programming: Principles and Practice using C++](http://www.stroustrup.com/programming.html) 中的词汇表）。

有关 C++ 的许多主题的更多信息，可以在[标准 C++ 基金会的网站](https://isocpp.org/) 找到。

- *ABI*: 应用二进制接口，对于特定硬件平台与操作系统的组合的一种规范。与 API 相对。
- *抽象类（abstract class）*: 不能直接用于创建对象的类；通常用于为派生类定义接口。 当类带有纯虚函数或只有受保护的构造函数时，它就是抽象的。
...
- *近似（approximation）*: 事物（比如值或者设计），接近于完美的或者理想的（值或设计）。 通常近似都是在理想情形中进行各种权衡的结果。
...
- *复杂度（complexity）*: 对某个问题构造解决方案的难度，或解决方案自身的一种难于精确定义的记法或度量。 有时候复杂度只是（简单地）表示对执行某个算法所需操作的数量的估计。
...
- *构造函数（constructor）*: 初始化（“构造”）一个对象的操作。 通常构造函数会建立起不变式，并且通常会获取对象被使用时所需的资源（并通常将由析构函数所释放）。
- *容器（container）*: 持有一些元素（其他对象）的对象。
- *复制（copy）*: 制造两个对象使其值比较为相等的操作。另见移动。
- *正确性（correctness）*: 如果程序或程序片段符合其说明，则其为正确的。 不幸的是，说明可能不完整或不一致，或者也可能无法满足用户的合理预期。 因此为了产生可接受的代码，我们有时候比仅仅遵守形式说明要做更多的事。
- *成本（cost）*: 产生一个程序，或者执行它的耗费（如开发时间，执行时间或空间等）。 理想情况下，成本应当是复杂度的函数。
- *定制点（customization point）*:
- *数据（data）*: 计算中所用到的值。
- *调试（debugging）*: 寻找并移除程序中的错误的行为；通常远没有测试那样系统化。
- *声明式（declaration）*: 程序中对一个名字及其类型的说明。
- *定义式（definition）*: 实体的声明式，提供了程序使用该实体所需的所有信息。 简化版定义：分配了内存额声明。
...
- *泛型编程（generic programming）*: 关注于算法的设计和高效实现的一种编程风格。 泛型算法能够对所有符合其要求的参数类型正确工作。在 C++ 中，泛型编程通常使用模板进行。
- *全局变量（global variable）*: 技术上说，命名空间作用域中的具名对象。
- *句柄（handle）*: 一个类，允许通过一个成员指针或引用来访问另一个对象。另见资源，复制，移动。
- *头文件（header）*: 包含用于在程序的各个部分中共享接口的声明的文件。
- *隐藏（hiding）*: 防止一个信息片段被直接看到或访问的行为。 例如，嵌套（内部）作用域中的名字会防止外部（外围）作用域中相同的名字被直接使用。
...
- *无限递归（infinite recursion）*: 无法终止的递归，直到机器耗尽内存无法维持其调用。 在现实中这种递归不可能是无限的，它会因某种硬件错误而终止。
...
- *正规*: 可以进行相等性比较的半正规类型（参见 `std::regular` 概念）。进行复制之后，副本对象与原对象比较为相等。正规类型的行为与如 `int` 这样的内建类型相似，且可以用 `==` 进行比较。 特别是，正规类型的对象可以进行复制，且复制的结果是与原对象比较为相等的一个独立对象。另见*半正规类型*。
...
这是我们的未完成列表。 以下各条目最终将成为规则或者规则的一部分。 或者，我们也会决定不需要做出改动并将条目移除。
...
- [Stroustrup14]: B. Stroustrup. [A Tour of C++](http://www.stroustrup.com/Tour.html). Addison Wesley 2014.
- [Stroustrup15]: B. Stroustrup, Herb Sutter, and G. Dos Reis: [A brief introduction to C++’s model for type- and resource-safety](https://github.com/isocpp/CppCoreGuidelines/blob/master/docs/Introduction to type and resource safety.pdf).
...
This site is open source. [Improve this page](https://github.com/lynnboy/CppCoreGuidelines-zh-CN/edit/master/CppCoreGuidelines-zh-CN.md).